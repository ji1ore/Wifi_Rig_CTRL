...
void setup()
{
  ...
  es8388.setSampleRate((es_sample_rate_t)48000);
...
void streamTask(void *param)
{
  ...
  {
    Serial.println("SamplingRate=0 → streamTask を開始しません");
    connected = false;
    streamTaskHandle = nullptr;
    vTaskDelete(NULL);
    return;
  ...
  updateSampleRate(samplingRates[selSampling]);
  connected = true;
...
void stopPlayback()
{
  ...
  {
    Serial.println("SamplingRate=0 → stopPlayback() で I2S 無効化スキップ");
  }
