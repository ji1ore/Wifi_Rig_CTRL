...
void handleRigConnectScreen()
{
    ...
    if (x >= 130 && x <= 310 && y >= 200 && y <= 235)
    {
        ...
        // --- I2S 初期化（SamplingRate=0 のときは無効化） ---
        SRate = samplingRates[selSampling];
        speakerEnabled = (SRate != 0);

        i2s_driver_uninstall(I2S_NUM_0); // 既存ドライバがあれば削除

        if (SRate > 0)
        {
            // ★ 有効なサンプリングレートのときだけ I2S を初期化
            i2s_config.sample_rate = SRate;
        ...
        {
            Serial.println("SamplingRate=0 → I2S 無効化");
        }
