...
wget https://raw.githubusercontent.com/ji1ore/M5CoreHamCAT/main/v1.20/RaspberryPiSetup/setup_netwk.sh
...
wget https://raw.githubusercontent.com/ji1ore/M5CoreHamCAT/main/v1.20/RaspberryPiSetup/setup_fastapi_radio.sh
...
⑥APRSについて
v1.20より、RaspberryPi上にDireWolfを導入してAPRSの送信を行えるようにしました。
APRSの設定はM5側のAPRSボタン長押しにより行います。

⑦APRSで使うGPS情報について
APRSで使うGPSについては、スマートフォンによる送付を行います(M5Coreが接続するWifiと同一ネットワーク上であることを前提とします)。
スマートフォンアプリ「Tasker(有償)」を使用し、RaspberryPi上のFastAPIに対してGPSの位置情報をHTTP POSTすることにより送付します。

Taskerの設定は以下の通りです。
・プロファイル：Tickにより指定時間ごとにGPS位置情報を送付
・タスク：
　①現在地取得(Get Loccation V2)
　②HTTPリクエスト(POST)
　　URL：http://raspizero:8000/gps
　　本文：{"lat":%gl_latitude,"lon":%gl_longitude}
　　※%gl_latitude/%gl_longitudeは現在地取得(Get Loccation V2)で取得した値
