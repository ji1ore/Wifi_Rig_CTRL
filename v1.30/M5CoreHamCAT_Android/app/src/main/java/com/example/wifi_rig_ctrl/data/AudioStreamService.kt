                ...
                try {
                    ...
                    call.execute().use { response ->
                        if (!response.isSuccessful) {
                            lastError = "音声ストリーム接続失敗 (${response.code})"
                        ...
                        if (prebuffered == 0) {
                            lastError = "音声データなし (attempt=$attempt, ${System.currentTimeMillis()-t0}ms)"
                    ...
                    if (!stopping) {
                        lastError = "音声接続エラー: ${e.message}"
                        Log.e("AudioStream", "attempt $attempt: ${e.message}")
                    } else {
                        break
                    }
                }
                if (streamed) break  // 再生成功 → リトライ不要
            }
