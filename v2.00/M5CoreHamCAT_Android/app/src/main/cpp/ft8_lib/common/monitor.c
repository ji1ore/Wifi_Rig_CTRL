...
void monitor_process(monitor_t* me, const float* frame)
{
    ...
    // Heap-allocate analysis buffers once and reuse across time_osr iterations.
    // Avoids stack-allocated VLAs (C99) which can overflow small-stack decode threads.
    kiss_fft_scalar* timedata = (kiss_fft_scalar*)malloc(me->nfft * sizeof(kiss_fft_scalar));
    kiss_fft_cpx*   freqdata = (kiss_fft_cpx*)malloc((me->nfft / 2 + 1) * sizeof(kiss_fft_cpx));
    if (!timedata || !freqdata) { free(timedata); free(freqdata); return; }
