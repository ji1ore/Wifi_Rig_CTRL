...
class AudioStreamService {
    ...
    // CWデコード用: PCM16サンプルをリアルタイムで通知（IOスレッドから呼ばれる）
    @Volatile var audioSampleListener: ((ShortArray) -> Unit)? = null

    // スパイクノイズ除去: 直近振幅のEMAに対して極端に大きいサンプルを無音化
    private var spikeEma = 1000f
            ...
            for (attempt in 1..MAX_RETRY) {
                ...
                try {
                    ...
                    call.execute().use { response ->
                        ...
                        spikeEma = 1000f  // EMAをリセット

                        val actualRate = response.header("X-Audio-Rate")?.toIntOrNull() ?: sampleRate
                        if (actualRate != sampleRate) Log.i("AudioStream", "X-Audio-Rate=$actualRate (requested $sampleRate)")
    ...
    private fun removeSpikes(buf: ByteArray, n: Int) {
        var i = 0
        while (i < n - 1) {
            val sample = ((buf[i + 1].toInt() shl 8) or (buf[i].toInt() and 0xFF)).toShort().toInt()
            val abs = if (sample < 0) -sample.toFloat() else sample.toFloat()
            if (abs > spikeEma * SPIKE_RATIO && spikeEma > SPIKE_MIN_EMA) {
                buf[i] = 0; buf[i + 1] = 0
            } else {
                spikeEma += (abs - spikeEma) * SPIKE_ALPHA
            }
            i += 2
        }
    }

    private fun notifySamples(buf: ByteArray, n: Int) {
        val listener = audioSampleListener ?: return
        if (n < 2) return
        val count = n / 2
    ...
    companion object {
        ...
        private const val SPIKE_ALPHA = 0.001f   // EMA追従速度 (~1000サンプル、ゆっくり追従)
        private const val SPIKE_RATIO = 5f        // EMAの5倍を超えたらスパイク
        private const val SPIKE_MIN_EMA = 200f    // 静音時の誤検知防止しきい値
    }
}
