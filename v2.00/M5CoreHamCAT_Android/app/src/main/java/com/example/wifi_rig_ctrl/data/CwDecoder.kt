...
class CwDecoder(private val sampleRate: Int = 8000) {
    ...
    private val HOP_SIZE  = 128             // 時間分解能は16msのまま維持 (75%オーバーラップ)
    ...
    private val FREQ_TOL      = 4     // ビン単位の周波数マッチング許容幅 (±62Hz)
    private val DRIFT_RADIUS  = 6     // チャンネルが追跡できる周波数ドリフト幅 (±94Hz)
    private val PEAK_SNR_MIN  = 5f   // 10→5: 弱信号でもチャンネル作成 (~7dB above noise)
    ...
    private val NOISE_FLATNESS_THRESH = 0.35f  // 0.5→0.35: 弱信号をノイズと誤判定しない
    ...
    private inner class Channel(var freqBin: Int, seedSigE: Float, seedNoiseE: Float) {
        ...
        private val ELEM_FLUSH_THRESH = maxOf(3, 500 / msPerHop)  // 500ms: 途切れ後に未確定要素を強制クリア
        ...
        init {
            ...
            if (seedSigE > seedNoiseE) {
                threshOn  = seedNoiseE + (seedSigE - seedNoiseE) * 0.50f  // 0.60→0.50: 弱信号対応
                threshOff = seedNoiseE + (seedSigE - seedNoiseE) * 0.22f  // 0.28→0.22: ヒステリシス維持
            }
        ...
        fun process(energy: Float, ci: Int) {
            ...
            // 500ms以上の途切れで未確定要素を強制クリア — 復帰後に前のバッファが混入しない
        ...
        private fun updateThresh(valid: Int) {
            // 途切れ中は閾値を更新しない — ノイズで履歴が埋まると閾値がフロア近くまで下がり
            // 復帰後に全ノイズをキーONと誤判定する原因になる
