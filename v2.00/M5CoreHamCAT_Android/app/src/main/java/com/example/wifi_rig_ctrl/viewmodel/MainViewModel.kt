...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    val tunerEnabled = MutableLiveData(false)
    val tunerSupported = MutableLiveData(false)
    private var tunerPollCounter = 0
    ...
    @Volatile private var cwFmTxGeneration = 0  // テールジョブの競合防止用世代カウンタ
    ...
    // Pi clock offset: piTimeMs = System.currentTimeMillis() + piClockOffsetMs
    // Synced in connectRig(). Negative when Android clock is ahead of Pi (NTP).
    @Volatile var piClockOffsetMs: Long = 0L
    // FT8 timing correction derived from decoded dt values (ms, applied on top of piClockOffsetMs)
    @Volatile var ft8ClockAdjMs: Long = 0L
    // FT8 TX active flag: prevents status observer from starting audioTx during FT8 TX
    @Volatile var ft8TxActive = false
    @Volatile var ft8FragmentActive = false  // FT8画面表示中: CWストリーム停止・二重起動防止

    // Status
    val errorMessage = MutableLiveData<String?>(null)
    val audioError = MutableLiveData<String?>(null)
    val isConnectedToRig = MutableLiveData(false)
    val piUpdateNeeded = MutableLiveData(false)
    val piApiVersion = MutableLiveData("")
    private var piVersionChecked = false
    ...
    companion object {
        ...
        const val EXPECTED_API_VERSION = "1.82"
    }
    ...
    suspend fun skipConnect(): String? {
        ...
        if (result == null) {
            piVersionChecked = false
            piUpdateNeeded.value = false
            isConnectedToRig.value = true
        }
        return result
    }
    ...
    suspend fun connectRig(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            if (alsaDev.isNotEmpty()) prefs.alsaDevice = alsaDev

            saveCurrentToProfile()

            api.openRig(rig.id, cat, baud, ptt, pttType)
            // ALSAデバイスが指定されていればPi側に設定 (FT8/SPK/TX全てに適用)
            if (alsaDev.isNotEmpty()) api.setAudioDevice(alsaDev, alsaDev)
            delay(1000)
            ...
            repeat(50) {
                ...
                if (st != null) {
                    ...
                    api.setPtt(false)
                    api.setPoll(true)
                    // スプリット解除: アプリ起動時にシングル周波数モードにする
                    api.setSimplex()
                    // Pi時刻同期: CWタイムスタンプのクロック差を補正する
                    val offset = api.getPiClockOffsetMs()
                    if (offset != null) {
                        piClockOffsetMs = offset
                        android.util.Log.i("PiClock", "Pi clock offset: ${offset}ms (Android is ${-offset}ms ahead of Pi)")
                    } else {
                        android.util.Log.w("PiClock", "Pi clock sync failed (Pi may need update: bash ~/create_api.sh)")
                    }
    ...
    private suspend fun updateStatus() {
        ...
        // Pi APIバージョン表示のみ (自動UPDATE無効化)
        if (st.apiVersion.isNotEmpty()) piApiVersion.value = st.apiVersion
        ...
        // Tuner state poll every 2s (10 × 200ms)
        if (++tunerPollCounter >= 10) {
            tunerPollCounter = 0
            val tuner = withContext(Dispatchers.IO) { api.getTunerState() }
            if (tuner != null) {
                tunerSupported.value = tuner.first
                if (tuner.first) tunerEnabled.value = tuner.second
                else tunerEnabled.value = false
            }
        }

        val pttOffCooldown = System.currentTimeMillis() - lastAndroidPttOffTime < 3000
        if (pttOffJob == null && audioTxStartJob == null && !pttOffCooldown && !cwFmTxActive && !ft8TxActive && aprsActive.value != true) {
    ...
    fun resetPiVersionCheck() {
        piVersionChecked = false
        piUpdateNeeded.value = false
    }

    fun updatePiSoftware() = viewModelScope.launch {
        val ctx = getApplication<Application>()
        val content = try {
            ctx.assets.open("api.py").readBytes()
        } catch (e: Exception) {
            errorMessage.value = "Asset read failed: ${e.message}"
            return@launch
        }
        val ok = withContext(Dispatchers.IO) { api.sendApiUpdate(content) }
        if (!ok) {
            errorMessage.value = "Pi update failed"
            return@launch
        }
        // Pi restarting — wait then reconnect
        delay(12_000L)
        val status = withContext(Dispatchers.IO) {
            var found: RigStatus? = null
            for (i in 0 until 10) {
                val st = api.getStatus()
                if (st != null) { found = st; break }
                delay(1500)
            }
            found
        }
        if (status?.apiVersion == EXPECTED_API_VERSION) {
            piUpdateNeeded.value = false
            // Pi再起動でオーディオストリームが切れるので再開する
            if (spkEnabled.value == true) {
                stopAudio()
                startAudio()
            }
        } else {
            errorMessage.value = "Pi update may have failed (ver=${status?.apiVersion})"
        }
    }
    ...
    private fun updateCwAudioStreamForMode(mode: String) {
        ...
        if (ft8FragmentActive) return  // FT8中はCWストリーム管理しない (audio_tx/ffmpeg競合防止)
        val isCwMode = mode.contains("CW", ignoreCase = true)
        if (isCwMode) cwUsb.stopCwAudioStream()
        else cwUsb.ensureCwAudioStream(api, apiKey.value ?: "")  // 既存ストリームは停止しない
    }

    fun stopCwAudioStream() = cwUsb.stopCwAudioStream()

    fun restartCwAudioStreamForCurrentMode() {
        val mode = sharedMode.value ?: return
        updateCwAudioStreamForMode(mode)
    }
    ...
    suspend fun saveAprsConfig(cfg: AprsConfig): Boolean {
        // aprsEnabled は saveFromUI() で設定済み。ここでは上書きしない
        // prefs は通信結果に関係なく保存（ローカル設定はオフライン時も維持）
    ...
    private fun cwFmKeyOn() {
        cwFmTailJob?.cancel()
        cwFmTailJob = null
        cwFmTxGeneration++  // テールジョブのsetPtt(false)を無効化
        // セッション継続中(テールキャンセル): ストリームはそのまま維持してリターン
        if (cwFmTxActive) return
        cwFmTxActive = true
        txEnabled.postValue(true)
        audioTxStartJob?.cancel()  // status observer由来のmic TX起動をキャンセル
        audioTxStartJob = null
        cwFmPttJob?.cancel()
        cwFmPttJob = viewModelScope.launch(Dispatchers.IO) {
            audio.stop()  // SPKを停止
            // 新セッション: PTT OFF後にaplayが殺されてストリームが切れているため作り直す
            cwUsb.stopCwAudioStream()
            cwUsb.startCwAudioStream(api, apiKey.value ?: "")
            api.setPoll(false)
            delay(80L + (cwFmDelayMs.value ?: 0).toLong())
            api.setPtt(true)
            withContext(Dispatchers.Main) { startPttHeartbeat() }
        }
    }
    ...
    private fun cwFmKeyOff() {
        if (!cwFmTxActive) return
        cwFmTailJob?.cancel()
        val myGen = cwFmTxGeneration
        cwFmTailJob = viewModelScope.launch {
            delay(500)
            if (cwFmTxGeneration != myGen) return@launch  // 新しいkeyOnで無効化済み
            stopPttHeartbeat()
            cwFmPttJob?.cancel()
            cwFmPttJob = null
            ...
            withContext(Dispatchers.IO) {
                delay(60)
                if (cwFmTxGeneration == myGen) {  // 競合チェック: 新keyOnが来ていなければ実行
                    api.setPtt(false)
                    api.setPoll(true)
                }
            }
            if (cwFmTxGeneration != myGen) return@launch
            cwFmTxActive = false
            txEnabled.value = false
            cwFmTailJob = null
            if (spkEnabled.value == true) startAudio()
        ...
        cwUsb.onKeyStateChange = { isOn, rawPacket ->
            ...
            if (cwUsbEnabled.value == true) {
                when {
                    isCwMode -> {
                        val now = System.currentTimeMillis()
                        val fireMs = now + piClockOffsetMs + prefs.cwDelayMs.toLong()
                        android.util.Log.d("CwKey", "${if (isOn) "ON " else "OFF"} now=$now offset=$piClockOffsetMs fire=$fireMs delay=${prefs.cwDelayMs}")
