...
# pi ユーザーが fastapi を passwordless sudo で再起動できるよう設定
echo "pi ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart fastapi" | sudo tee /etc/sudoers.d/pi-fastapi-restart > /dev/null
sudo chmod 440 /etc/sudoers.d/pi-fastapi-restart
echo "sudoers 設定完了 (pi: systemctl restart fastapi)"
...
_ft8_build_lock   = threading.Lock()
_ft8_running      = False
_ft8_is_ft4       = False
_ft8_thread       = None
_ft8_results      = []
...
# ─── ft8wav/ft8enc: mfsk-core (Rust) のみ使用 — C binary (ft8_decode/ft8_encode) は廃止 ───
FT8LIB_DIR="/home/pi/ft8_lib"  # ft8wav Cargoプロジェクト内で参照


cat << 'FT8CSRC_REMOVED' > /dev/null
/* REMOVED: ft8_decode.c (C binary) — ft8wav (Rust) のみ使用
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static bool noop_lookup(ftx_callsign_hash_type_t t, uint32_t h, char *c)
{ if (c) c[0]='\0'; return false; }
static void noop_save(const char *c, uint32_t h) { (void)c; (void)h; }
static ftx_callsign_hash_interface_t kHashIf = { noop_lookup, noop_save };

static int read_wav(const char *path, float **out, int *rate)
{
    FILE *f = fopen(path, "rb");
    if (!f) return -1;
    char tag[4]; uint32_t sz;
    uint16_t channels=0, bits=0; uint32_t sample_rate=0;
    if (fread(tag,1,4,f)!=4 || memcmp(tag,"RIFF",4)) { fclose(f); return -1; }
    fread(&sz,4,1,f);
    if (fread(tag,1,4,f)!=4 || memcmp(tag,"WAVE",4)) { fclose(f); return -1; }
    while (fread(tag,1,4,f)==4 && fread(&sz,4,1,f)==1) {
        if (!memcmp(tag,"fmt ",4)) {
            uint8_t b[40]; uint32_t rd=sz<40?sz:40;
            fread(b,1,rd,f); if (sz>rd) fseek(f,sz-rd,SEEK_CUR);
            channels    =(uint16_t)(b[2] |b[3] <<8);
            sample_rate =(uint32_t)(b[4] |b[5] <<8|b[6] <<16|b[7] <<24);
            bits        =(uint16_t)(b[14]|b[15]<<8);
        } else if (!memcmp(tag,"data",4)) {
            if (channels!=1||bits!=16) { fclose(f); return -1; }
            int n=(int)(sz/2);
            int16_t *raw=(int16_t*)malloc(n*sizeof(int16_t));
            if (!raw) { fclose(f); return -1; }
            fread(raw,2,n,f); fclose(f);
            float *s=(float*)malloc(n*sizeof(float));
            if (!s) { free(raw); return -1; }
            for (int i=0;i<n;i++) s[i]=raw[i]/32768.0f;
            free(raw); *out=s; *rate=(int)sample_rate; return n;
...
{
    int is_ft4=0; const char *wav=NULL;
    for (int i=1;i<argc;i++) {
        if (!strcmp(argv[i],"--ft4")) is_ft4=1;
        else wav=argv[i];
    }
    if (!wav) { fprintf(stderr,"Usage: %s [--ft4] input.wav\n",argv[0]); return 1; }

    float *samples; int sr;
    int n=read_wav(wav,&samples,&sr);
    if (n<0) { fprintf(stderr,"Cannot read WAV: %s\n",wav); return 1; }

    monitor_config_t cfg; memset(&cfg,0,sizeof(cfg));
    cfg.sample_rate=sr; cfg.f_min=200.0f; cfg.f_max=3200.0f;
    ...
    if (mon.block_size<=0||!mon.wf.mag)
    { fprintf(stderr,"monitor_init failed\n"); free(samples); return 1; }

    for (int off=0;off+mon.block_size<=n;off+=mon.block_size)
        monitor_process(&mon,samples+off);
    free(samples);

    ftx_candidate_t cands[500];
    int nc=ftx_find_candidates(&mon.wf,500,cands,4);
    for (int i=0;i<nc;i++) {
        ftx_message_t msg; ftx_decode_status_t st;
        memset(&msg,0,sizeof(msg)); memset(&st,0,sizeof(st));
        if (!ftx_decode_candidate(&mon.wf,&cands[i],80,&msg,&st)) continue;
        char text[FTX_MAX_MESSAGE_LENGTH+1]; ftx_message_offsets_t offsets;
        memset(text,0,sizeof(text)); memset(&offsets,0,sizeof(offsets));
        ftx_message_decode(&msg,&kHashIf,text,&offsets);
        float bin_hz2 = (float)sr / (float)mon.block_size;
        int cand_freq_hz2 = (int)(cfg.f_min + (cands[i].freq_offset + (float)cands[i].freq_sub / cfg.freq_osr) * bin_hz2);
        printf("%4d %5.1f %4d ~ %s\n",(int)(cands[i].score*0.5f-30.0f),st.time,cand_freq_hz2,text);
    }
    monitor_free(&mon);
    return 0;
}
...
# ft8_decode (C binary) build removed — ft8wav (Rust) のみ使用

cat << 'FT8CPPSRC_REMOVED' > /dev/null
// ft8_encode_main.cpp REMOVED — ft8enc (Rust) のみ使用
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
// ft4/encode.h may not be present in all ft8_lib versions — declare directly
extern "C" void ft4_encode(const uint8_t* payload, uint8_t* tones);

static bool noop_lookup(ftx_callsign_hash_type_t t, uint32_t h, char* c)
{ (void)t; (void)h; if (c) c[0]='\0'; return false; }
static void noop_save(const char* c, uint32_t h) { (void)c; (void)h; }
...
# ft8_encode (C binary) build removed — ft8enc (Rust) のみ使用

# ─── ft8wav: mfsk-core (webft8) Rust FT8/FT4 デコーダをビルド ───
FT8WAV_BIN="/usr/local/bin/ft8wav"
FT8WAV_DIR="$HOME/ft8wav"

# Rust/cargo が無ければ .cargo/env を試み、それでもなければ rustup でインストール
if ! command -v cargo &>/dev/null; then
    [ -f "$HOME/.cargo/env" ] && source "$HOME/.cargo/env"
fi
if ! command -v cargo &>/dev/null; then
    echo "Rust をインストール中..."
    curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain stable --no-modify-path || echo "rustup インストール失敗"
    [ -f "$HOME/.cargo/env" ] && source "$HOME/.cargo/env" || true
fi

mkdir -p "$FT8WAV_DIR/src"

cat << 'FT8WAVTOML' > "$FT8WAV_DIR/Cargo.toml"
[package]
name = "ft8wav"
version = "0.1.0"
edition = "2021"
...
cat << 'FT8WAVRS' > "$FT8WAV_DIR/src/main.rs"
use std::env;
use hound::WavReader;

fn main() {
    let args: Vec<String> = env::args().collect();
    let is_ft4 = args.iter().any(|a| a == "--ft4");
    let wav_path = match args.iter().skip(1).find(|a| !a.starts_with('-')) {
        Some(p) => p.clone(),
        None => {
            eprintln!("Usage: ft8wav [--ft4] <file.wav>");
            std::process::exit(1);
        }
    };
    let mut reader = WavReader::open(&wav_path)
        .unwrap_or_else(|e| { eprintln!("open WAV: {e}"); std::process::exit(1) });
    ...
    if is_ft4 {
        ...
        // message77() returns &[u8]; convert to &[u8; 77] for unpack77
        for r in decode_frame(&samples, 200.0, 3000.0, 0.0, 400) {
            if let Ok(arr) = <&[u8; 77]>::try_from(r.message77()) {
                if let Some(text) = unpack77(arr) {
                    println!("{:4} {:5.1} {:4} ~ {}", r.snr_db as i32, r.dt_sec, r.freq_hz as i32, text);
                }
            }
        }
    } else {
        ...
        use mfsk_core::ft8::message::unpack77;
        // message77 is a [u8; 77] field; pass &r.message77 directly
        for r in decode_frame(&samples, 200.0, 3000.0, 0.0, None, DecodeDepth::BpAllOsd, 400) {
...
cat << 'FT8ENCRS' > "$FT8WAV_DIR/src/ft8enc.rs"
use std::env;
use std::io::Write;
use mfsk_core::ft8::message::{pack77, pack77_free_text};

fn pack_message(msg: &str) -> [u8; 77] {
    let parts: Vec<&str> = msg.split_whitespace().collect();
    if parts.len() == 3 {
        if let Some(bits) = pack77(parts[0], parts[1], parts[2]) {
            return bits;
        }
    }
    if let Some(bits) = pack77_free_text(msg) {
        return bits;
    }
    eprintln!("Cannot encode message: {msg}");
    std::process::exit(1);
}
...
fn main() {
    let args: Vec<String> = env::args().collect();
    if args.len() < 3 {
        eprintln!("Usage: ft8enc <message> <base_hz> [--ft4]");
        std::process::exit(1);
    }
    let msg = &args[1];
    let f0: f32 = args[2].parse().unwrap_or(1500.0);
    let is_ft4 = args.iter().any(|a| a == "--ft4");

    let bits: [u8; 77] = pack_message(msg);
    let amplitude: i16 = 30000;

    let pcm: Vec<i16> = if is_ft4 {
        use mfsk_core::ft4::encode::{message_to_tones, tones_to_i16};
        let tones = message_to_tones(&bits);
        tones_to_i16(&tones, f0, amplitude)
    } else {
    ...
    for s in &pcm {
        out.write_all(&s.to_le_bytes()).expect("write failed");
    }
}
FT8ENCRS

echo "ft8wav/ft8enc をビルド中 (初回は数分かかります)..."
if command -v cargo &>/dev/null; then
    if (cd "$FT8WAV_DIR" && cargo build --release 2>&1); then
        sudo cp "$FT8WAV_DIR/target/release/ft8wav" "$FT8WAV_BIN"
        sudo cp "$FT8WAV_DIR/target/release/ft8enc" "/usr/local/bin/ft8enc"
        echo "ft8wav/ft8enc ビルド成功: $FT8WAV_BIN, /usr/local/bin/ft8enc"
    else
        echo "ft8wav/ft8enc ビルド失敗 (詳細は上のログ参照)"
    fi
else
    echo "cargo が見つかりません。ft8wav/ft8enc のビルドをスキップします。"
fi

# USB Audio CODEC Playback ボリュームを最大に設定
echo "ALSA CODEC Playback ボリューム設定..."
for ctrl in Speaker Headphone PCM Master \
            "Speaker Playback Volume" "Headphone Playback Volume" \
            "PCM Playback Volume" "Master Playback Volume"; do
    amixer -c CODEC sset "$ctrl" 100% unmute 2>/dev/null && echo "  OK: $ctrl=100%"
done
echo "現在のCODECコントロール:"
amixer -c CODEC scontents 2>/dev/null | grep -E "(Playback|Capture|Volume|Switch)" | head -20
