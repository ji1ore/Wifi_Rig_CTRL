...
| FT8 / FT4 | WebView ベースの受信デコード・送信（実験的機能） |
...
同フォルダの `Wifi_RIG_CTRL_v2.01_debug.apk` を端末にインストールしてください。

### ソースからビルドする場合

1. [Android Studio](https://developer.android.com/studio) をインストール
2. このフォルダをプロジェクトとして開く
3. **Run → Run 'app'** で端末に直接インストール
...
長押しで CW デコード表示の ON/OFF を切り替えます。
...
SPK ボタン長押しで CW デコードを有効にすると、受信音声から CW 符号をリアルタイムでデコードします。

- 最大 5 チャンネル同時追跡（300〜3000 Hz）
- 速度自動追従（適応 dit 長推定）
- デコードエリア長押しでマルチ RX 表示に切り替え可能
...
APRS 設定画面でコールサイン・SSID・パス・間隔・周波数を設定後、**APRS** ボタンで送信開始します。  
GPS 座標または手動座標を選択できます。

---

## Raspberry Pi セットアップ

`../RaspberryPiSetup/` 内のスクリプトを使用してください。詳細は同フォルダの `readme.txt` を参照。
