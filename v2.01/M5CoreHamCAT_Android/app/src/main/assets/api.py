...
_mgr_sub = _FfmpegMgr(af="volume=8.0", kill_direwolf=False, idle_stop_sec=1)
...
async def admin_update_cw_bridge(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"Syntax error: {e}")
    bridge_path = Path("/home/pi/cw_bridge.py")
    bak_path = Path("/home/pi/cw_bridge.py.bak")
    try:
    ...
    with _cw_bridge_lock:
        ...
        if saved_port:
            try:
                venv_py = "/home/pi/fastapi/bin/python3"
                _cw_bridge_proc = subprocess.Popen([venv_py, "/home/pi/cw_bridge.py", saved_port])
