...
def poll_signal():
    ...
    while True:
        ...
        if time.time() - last_bkin > 5:
            try:
                for _func in ("SBKIN", "FBKIN"):
                    raw = rigctl_cmd(f"u {_func}")
                    _v = raw.split()[0] if raw else ""
                    if _v.lstrip("-").isdigit():
                        radio_cache["bk_in"] = int(_v)
                        break
            except Exception:
                pass
            last_bkin = time.time()
...
def set_bk_in(state: int = Form(...)):
    try:
        cmd = "U SBKIN " + str(state)
        raw = rigctl_cmd(cmd)
        ok = raw is not None and raw.strip() == "RPRT 0"
        if ok:
            radio_cache["bk_in"] = state
        return {"ok": ok, "bk_in": state, "raw": str(raw)}
    except Exception as e:
...
async def audio_tx(request: Request, rate: int = 8000, ptt: int = 0):
    ...
    if ptt:
        # FT8 TX: 排他的ALSA使用 — 全プロセス停止+ffmpegでレートコンバート
        def _kill_procs_ft8():
            subprocess.run(["pkill", "-9", "direwolf"], capture_output=True)
            subprocess.run(["pkill", "-9", "aplay"],    capture_output=True)
            for mgr in (_mgr_sub, _mgr_rx):
                p = mgr.proc
                mgr.proc = None
                if p is not None:
                    try: p.kill()
        ...
        # ffmpeg: webft8 は 12000Hz など任意レートで送るためレートコンバート必須
        proc = subprocess.Popen(
            ["ffmpeg", "-loglevel", "error",
             "-f", "s16le", "-ar", str(rate), "-ac", "1", "-i", "pipe:0",
             "-ar", "32000", "-f", "alsa", _alsa_playback_dev],
        ...
        # capture ffmpegはkill不要 — USB CODECはキャプチャと再生を同時使用可能
        def _kill_voice():
            subprocess.run(["pkill", "-9", "direwolf"], capture_output=True)
            subprocess.run(["pkill", "-9", "aplay"],    capture_output=True)
    ...
    while True:
        ...
        if len(data) == 5 and data[0] == 0xE0:
            ...
            if _server_offset_ms is not None:
                ...
                print(f"[cw_bridge] SYNC tunnel server_now_ms={server_now_ms}", flush=True)
            else:
                print(f"[cw_bridge] SYNC tunnel: no server offset yet, ignoring", flush=True)
