...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    suspend fun updatePiSoftware(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            // /admin/setup only exists in new api.py, so retry up to 3 times
            ...
            // api.py already succeeded. setup failure will be resolved on next UPDATE, treat as warning
    ...
    fun connectCwUsb(device: android.hardware.usb.UsbDevice) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // This coroutine runs on Dispatchers.IO so DNS blocking is acceptable
                ...
                // Auto-enable after successful USB connection
