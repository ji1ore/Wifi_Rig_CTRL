...
if sudo -n true 2>/dev/null; then
    ...
    echo "=== 初回のみ SSH で実行: sudo bash /home/pi/create_api.sh ==="
fi

if $_HAS_SUDO; then
    # pi ユーザーが sudo なしで systemctl restart できるよう NOPASSWD 設定
    echo "pi ALL=(ALL) NOPASSWD: /bin/systemctl restart fastapi, /bin/systemctl restart fastapi-audio, /bin/systemctl restart webft8" \
...
_webft8_dir = "/home/pi/webft8_static/web"
...
class _FfmpegMgr:
    ...
    def ensure(self, rate: str):
        with self._lock:
            if (self.proc and self.proc.poll() is None
                    and self._rate == rate
                    and self._device == _alsa_capture_dev):
                return self.proc
    ...
    def _start(self, rate: str):
        ...
        cmd = [
            ...
            "-ar", rate, "-i", _alsa_capture_dev, "-ac", "1",
        ]
        if self._af:
            cmd += ["-af", self._af]
        cmd += [
        ...
        self.proc = proc
        self._rate = rate
        self._device = _alsa_capture_dev
...
# FT8音声: 音量ブーストのみ（webft8が独自DSP）、direwolf停止なし
_mgr_sub = _FfmpegMgr(af="volume=8.0", kill_direwolf=False)
...
def _start_webft8_if_needed():
    web_dir = "/home/pi/webft8_static/web"
...
def audio_stream(request: Request, background_tasks: BackgroundTasks):
    ...
    _mgr_sub.stop()  # FT8用ffmpegを停止してALSAを解放
    proc = _mgr_rx.ensure(rate)

    # ffmpeg即死 = aplayとのALSA競合の場合、aplay解放まで待ってから再起動
...
async def audio_tx(request: Request, rate: int = 8000, ptt: int = 0):
    ...
    def _kill_procs():
        ...
        subprocess.run(["pkill", "-9", "ffmpeg"],   capture_output=True)  # ALSAデバイス解放
        _mgr_sub.proc = None   # FfmpegMgr に再起動を促す
        time.sleep(0.2)        # ALSA 解放待ち
    ...
    if ptt:
        ...
        radio_cache["tx"] = True
        last_ptt_state = 1
        last_heartbeat = time.time() + 60  # suppress watchdog for up to 60s TX
        await asyncio.sleep(0.5)           # PTT settle time
...
async def audio_sub(request: Request, background_tasks: BackgroundTasks):
    ...
    if proc.poll() is not None:
        ...
        time.sleep(0.1)
        if proc.poll() is not None:
            return FR(status_code=503, content="ffmpeg error")
...
async def admin_update(request: Request):
    ...
    def _restart():
        ...
        _time.sleep(0.5)
        # 方法1: sudo -n systemctl (NOPASSWD 設定済みの場合)
        r = subprocess.run(
            ["sudo", "-n", "systemctl", "restart", "fastapi", "fastapi-audio"],
            capture_output=True, timeout=10
        )
        if r.returncode == 0:
            return
        # 方法2: sudo 不可 → pkill + nohup で直接再起動 (同ユーザープロセスなので sudo 不要)
        restart_sh = (
            "#!/bin/bash\n"
            "sleep 2\n"
            ...
            "sleep 3\n"
            "cd /home/pi/fastapi\n"
            "/home/pi/fastapi/bin/uvicorn api:app --host 0.0.0.0 --port 8000 "
...
async def admin_setup(request: Request):
    ...
    if not content:
        raise HTTPException(status_code=400, detail="Empty body")
    script_path = Path("/home/pi/create_api.sh")
    try:
...
# ─── server.py を Python 3.13 対応版に上書き（SSL エラー抑制 + ALPN 固定）───
cat << 'SRVEOF' > /home/pi/webft8_static/web/server.py
#!/usr/bin/env python3
import http.server, ssl, sys, urllib.request
    ...
    def do_GET(self):
        if self.path.startswith('/audio_sub'):
            ...
            try:
                ...
                resp = urllib.request.urlopen(req_obj, timeout=None)
...
httpd = _Srv(('0.0.0.0', 8443), WebFt8Handler)
ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
ctx.load_cert_chain('./server.pem')
ctx.set_alpn_protocols(['http/1.1'])
httpd.socket = ctx.wrap_socket(httpd.socket, server_side=True)
print('webft8 :8443 started', flush=True)
httpd.serve_forever()
SRVEOF

# ─── coi-serviceworker.js を生成（crossOriginIsolated / SharedArrayBuffer 有効化）───
cat << 'COIEOF' > /home/pi/webft8_static/web/coi-serviceworker.js
...
COIEOF
echo "coi-serviceworker.js 生成完了"

# ─── webft8 HTTPS 用 自己署名証明書を生成（なければ）───
cd /home/pi/webft8_static/web
...
pkill -f "python3 /home/pi/webft8_static" 2>/dev/null || true
sleep 1
...
[Service]
User=pi
WorkingDirectory=/home/pi/webft8_static/web
...
if $_HAS_SUDO; then
    ...
    sleep 2
    cd /home/pi/fastapi
    nohup /home/pi/fastapi/bin/uvicorn api:app --host 0.0.0.0 --port 8000 \
        >> /tmp/uvicorn_restart.log 2>&1 &
