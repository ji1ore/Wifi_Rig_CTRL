...
class RigApiService(private var hostName: String, private var apiPort: Int, private var apiKey: String = "") {
    ...
    fun cwSendMorse(text: String, wpm: Int): Boolean = try {
    ...
    private fun buildAprsJson(cfg: AprsConfig): String {
        ...
        return """{"callsign":"${cfg.callsign}","ssid":${cfg.ssid},"path":"${cfg.path}","interval":${cfg.interval},"freq":${cfg.freq},"baud":${cfg.baud},"use_gps":${cfg.useGps},"manual_lat":${cfg.manualLat},"manual_lon":${cfg.manualLon},"symbol":"${cfg.symbol}","comment":"$safeComment","destination":"${cfg.destination}","sound_device":"${cfg.soundDevice}","rig_id":"${cfg.rigId}","cat_device":"${cfg.catDevice}"}"""
    }
}
