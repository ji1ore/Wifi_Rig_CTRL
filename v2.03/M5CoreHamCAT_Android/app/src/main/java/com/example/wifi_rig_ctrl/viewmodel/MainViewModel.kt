...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private suspend fun updateStatus() {
        ...
        // log10(10) denominator: raw=9 → s=9.0 (S9), gives more deflection for weak signals
        var s = 9f * (log10(raw + 1f) / log10(10f))
    ...
    suspend fun updatePiSoftware(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            if (setupErr == null) {
                ...
                // Wait a moment for systemd to finish the restart triggered by create_api.sh
                Thread.sleep(5000)
    ...
    fun sendCwText(text: String, wpm: Int) {
        ...
        cwTextJob = viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                if (isCwMode) {
                    delay(600) // rig_lock 解放待ち
                    val ok = api.cwSendMorse(normalized, wpm)
                    if (!ok) {
                        withContext(Dispatchers.Main) {
