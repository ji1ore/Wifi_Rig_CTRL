...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    val cwBt  = CwBtService(app).also  { it.sidetoneEnabled = prefs.cwSidetoneEnabled }
    ...
    // CW Bluetooth (Classic BT SPP: RemoteKeyer-BT)
    val cwBtConnected = MutableLiveData(false)
    val cwBtEnabled   = MutableLiveData(false)
    private var cwBtConnecting = false

    // CW BLE (Nordic UART Service: DualKey-BLE)
    ...
    fun updateCwWpm(wpm: Int) {
        ...
        if (cwBt.isConnected)  viewModelScope.launch(Dispatchers.IO) { cwBt.sendWpmToM5(wpm) }
    ...
    fun updatePaddleSwap(swapped: Boolean) {
        ...
        if (cwBt.isConnected)  cwBt.sendPaddleSwapToM5(swapped)
    ...
    private suspend fun updateStatus() {
        ...
        val cwBtCwActive  = (cwBtEnabled.value == true || cwBleEnabled.value == true) && cwMode
    ...
    @SuppressLint("MissingPermission")
    fun connectCwBt(device: BluetoothDevice) {
        if (cwBt.isConnected || cwBtConnecting) return
        cwBtConnecting = true

        cwBt.sidetoneEnabled = prefs.cwSidetoneEnabled
        cwBt.onConnectionStateChange = { connected ->
            cwBtConnected.postValue(connected)
            if (!connected) cwBtEnabled.postValue(false)
        }
        cwBt.onWpmChange = { wpm ->
            prefs.cwWpm = wpm
            cwWpm.postValue(wpm)
        }
        cwBt.onKeyStateChange = { isOn, rawPacket ->
            cwBt.currentMode = sharedMode.value ?: ""
            val mode = cwBt.currentMode
            val isCwMode = mode.contains("CW", ignoreCase = true)
            val isFmMode = mode.contains("FM", ignoreCase = true)
            if (cwBtEnabled.value == true) {
                when {
                    isCwMode -> {
                        val pkt = rawPacket.copyOf()
                        if (prefs.cwDelayMs > 0) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val ok = cwBt.connect(device)
                withContext(Dispatchers.Main) {
                    android.widget.Toast.makeText(
                        getApplication(),
                        if (ok) "CW BT connected to ${device.name}" else "CW BT connect failed",
                        android.widget.Toast.LENGTH_LONG
                    ).show()
                }
                if (!ok) { cwBtConnected.postValue(false); return@launch }
                if (cwSendThread?.isAlive != true) startCwSendThread()
                cwBt.keeyerWpm = prefs.cwWpm
                withContext(Dispatchers.Main) {
                    cwBtEnabled.value = true
                    startCwServerPolling()
                }
                cwBt.sendPaddleSwapToM5(prefs.cwPaddleSwap)
                val resolveHost = prefs.hostName
                if (resolveHost.isNotEmpty()) {
                    try {
                        val addr = java.net.InetAddress.getByName(resolveHost)
                        cwBt.piSyncAddress = java.net.InetSocketAddress(addr, CW_UDP_PORT)
                    } catch (e: Exception) {
                        android.util.Log.w("CwBt", "pre-resolve failed: ${e.message}")
                    }
                }
                try { api.cwOpen(prefs.cwPort, 0) } catch (_: Exception) {}
            } finally {
                withContext(Dispatchers.Main) { cwBtConnecting = false }
            }
        }
    }

    fun disconnectCwBt() {
        cwBtConnecting = false
        stopCwServerPolling()
        cwBt.piSyncAddress = null
        cwBt.disconnect()
        cwBtConnected.value = false
        cwBtEnabled.value = false
    }

    // ───────── CW BLE relay (DualKey-BLE) ─────────
