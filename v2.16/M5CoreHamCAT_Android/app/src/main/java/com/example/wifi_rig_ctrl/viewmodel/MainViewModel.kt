...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    companion object {
        ...
        const val EXPECTED_PI_API_VERSION = "2.12"
    }
    ...
    suspend fun connectToRasPi(): String? = withContext(Dispatchers.IO) {
        ...
        if (useCIV.value == true) {
            ...
            if (!ok) return@withContext "CI-V接続失敗\n${civ.lastError}"
    ...
    suspend fun connectRig(): String? = withContext(Dispatchers.IO) {
        ...
        if (useCIV.value == true) {
            if (!civ.isConnected) {
                ...
                val user = civUser.value ?: ""
                val pass = civPassword.value ?: ""
                val cm2 = getApplication<android.app.Application>()
                    .getSystemService(android.net.ConnectivityManager::class.java)
                val wifiNet2 = cm2?.allNetworks?.firstOrNull { net ->
                    cm2.getNetworkCapabilities(net)
                        ?.hasTransport(android.net.NetworkCapabilities.TRANSPORT_WIFI) == true
                }
                if (!civ.connect(host, port, port2, port3, user, pass, wifiNet2)) return@withContext "CI-V再接続失敗\n${civ.lastError}"
                acquireCivWifiLock()
            }
            val freq = civ.getFrequency() ?: 7_000_000L
            val mode = civ.getMode()
            val power = civ.getRfPower() ?: 1f
            val sql = civ.getSql() ?: 0f
            val bkIn = civ.getBkIn() ?: false
            val modelName = civ.getModelFromCivAddress()
            val modes = civ.getSupportedModes()
            withContext(Dispatchers.Main) {
    ...
    private suspend fun updateCivStatus() {
        val st = withContext(Dispatchers.IO) { civ.pollStatus() } ?: run {
            if (!civ.isConnected) {
                // attempt silent reconnect with 30s backoff (IC-705 needs ≥30s to fully release previous session)
    ...
    private suspend fun updateStatus() {
        ...
        if (pttOffJob == null && audioTxStartJob == null && !pttOffCooldown && !connectCooldown && !cwFmTxActive && !cwTextTxActive && !cwUsbCwActive && !cwBtCwActive && aprsActive.value != true) {
    ...
    fun fetchModeList() = viewModelScope.launch {
        ...
        if (useCIV.value == true) {
            supportedModes.value = civ.getSupportedModes()
            return@launch
        }
    ...
    fun sendFreq(freqHz: Long) {
        ...
        if (useCIV.value == true) {
            viewModelScope.launch(Dispatchers.IO) { civ.setFrequency(freqHz) }
        } else {
    ...
    fun sendMode(mode: String, width: Int) {
        ...
        if (isDemoMode.value == true) return
        if (useCIV.value == true) {
            viewModelScope.launch(Dispatchers.IO) { civ.setMode(mode, width) }
        } else {
    ...
    fun sendPower(value: Float) {
        ...
        if (isDemoMode.value == true) return
        if (useCIV.value == true) {
            viewModelScope.launch(Dispatchers.IO) { civ.setRfPower(value) }
        } else {
    ...
    fun sendBkIn(on: Boolean) {
        ...
        if (isDemoMode.value == true) return
        if (useCIV.value == true) {
            viewModelScope.launch(Dispatchers.IO) { civ.setBkIn(on) }
        } else {
    ...
    fun sendSQL(value: Float) {
        ...
        if (isDemoMode.value == true) return
        if (useCIV.value == true) {
            viewModelScope.launch(Dispatchers.IO) { civ.setSql(value) }
        } else {
    ...
    fun setPttEnabled(on: Boolean) {
        ...
        // CI-V mode: software PTT via CI-V (no Pi audio streaming)
        if (useCIV.value == true) {
            externalPttActive = false
            txEnabled.value = on
            viewModelScope.launch(Dispatchers.IO) { civ.setPtt(on) }
            return
        }
    ...
    // CI-V mode SPK control: toggle AudioTrack playback in CivTcpService
    fun civSetSpk(on: Boolean) { civ.spkEnabled = on }
        ...
        try {
            kotlinx.coroutines.coroutineScope {
                ...
                if (isCwMode) {
                    ...
                    if (bkIn) {
                        // BK-IN ON: use IC-705 internal keyer (CI-V 0x17 sendCwMessage).
                        // Set key speed to match sidetone, then send in ≤30-char chunks.
                        ...
                        // BK-IN OFF: PTT-per-element pseudo-CW (carrier ON/OFF per element).
                        val words = text.uppercase().trim().split(Regex("\\s+")).filter { it.isNotEmpty() }
                        for ((wi, word) in words.withIndex()) {
                    ...
                    // SSB/FM: stream CW PCM audio through RS-BA1 TX audio path
                    ...
                    var offset = 0
                    val chunkSize = 320  // 20ms at 8kHz
            ...
            withContext(NonCancellable) {
                if (isCwMode) { civ.stopCwMessage(); civ.civPttUp() }
            }
            withContext(Dispatchers.Main) {
    ...
    fun disconnectFromRig() {
        if (useCIV.value == true) { civ.disconnect(); releaseCivWifiLock() }
