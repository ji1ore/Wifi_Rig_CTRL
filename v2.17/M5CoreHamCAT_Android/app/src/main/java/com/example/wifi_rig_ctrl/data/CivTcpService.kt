...
class CivTcpService {
    ...
    private fun handlePingAndSendPong(r: ByteArray): Boolean {
        ...
        if (civRemoteId != 0L && senderId != civRemoteId) {
    ...
    fun connect(host: String, port1: Int, port2: Int, port3: Int,
        ...
        return try {
            ...
            ctrlSocket = try {
                val bindAddr = java.net.InetSocketAddress(localIp ?: InetAddress.getByName("0.0.0.0"), port1)
                DatagramSocket(null).also { s ->
                    s.reuseAddress = true; s.bind(bindAddr); network?.bindSocket(s)
                    Log.i(TAG, "ctrlSocket bound ${s.localAddress}:${s.localPort}")
                }
            } catch (e: Exception) {
                Log.w(TAG, "ctrlSocket fallback: ${e.message}")
                DatagramSocket(port1).also { network?.bindSocket(it) }
            }
            ...
            if (!hadKeptCivSocket) {
                civSocket = try {
                    val bindAddr = java.net.InetSocketAddress(localIp ?: InetAddress.getByName("0.0.0.0"), port2)
                    DatagramSocket(null).also { s ->
                        s.reuseAddress = true; s.bind(bindAddr); network?.bindSocket(s)
                    ...
                    DatagramSocket(port2).also { network?.bindSocket(it) }
                }
            ...
            if (!hadKeptCivSocket) {
                ...
                if (!gotCivIAH) { lastError = "IC-705 CIVソケット応答なし"; disconnect(); return false }
            } else {
