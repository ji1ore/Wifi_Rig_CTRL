...
final class CivService: @unchecked Sendable {
    ...
    var spkEnabled: Bool = false {
        didSet { if spkEnabled { tryPlayAudio() } else { audioPause() } }
    }
    ...
    private func recvFrom(_ fd: Int32, timeoutMs: Int) -> Data? {
        guard fd >= 0 else { return nil }
        var tv = timeval(tv_sec: time_t(timeoutMs / 1000),
    ...
    private func initAudioPlayer() {
        ...
        } catch { print("[CIV] AVAudioSession setup failed: \(error)") }
        #endif
        ...
        do {
            ...
            NSLog("[CIV] audioEngine started — mic active")
        } catch { print("[CIV] audioEngine start failed: \(error)") }
    ...
    private func scheduleAudioPCM(_ pcm: Data) {
        guard let player = audioPlayer, let fmt = audioFormat else { return }
        ...
        if spkEnabled && !player.isPlaying { player.play() }
    }
    ...
    func connect(host: String, port1: Int, port2: Int, port3: Int,
        ...
        while nowMs() < rsEnd, !gotStatus {
            ...
            case Self.STATUS_SIZE:
                ...
                if err == 0xFFFFFFFF { setErr(String(localized: "接続失敗 (Status FF)")); disconnect(); return false }
                gotStatus = true
        ...
        if !gotCivIAH { setErr(String(localized: "IC-705 CIVソケット応答なし")); disconnect(); return false }
        ...
        // Leave civStreamOpened = false (matching Android CivTcpService behaviour).
        // The first pollStatus() call will drainCivBroadcasts() (picking up any transceive
        // frequency broadcasts that accumulated during initAudioPlayer()) and then exchange()
        // will re-confirm the CI-V stream with an OpenClose before querying — this eliminates
        // the 3×1500 ms failure window that otherwise preceded the first successful frequency read.
    ...
    func setMode(_ mode: String, width: Int) -> Bool {
        ...
        let mc: Int = switch mode.uppercased() {
            ...
            case "CWR","CW-R": 0x07; case "RTTYR": 0x08; default: 0x01
        }
    ...
    func getSupportedModes() -> [String] {
        civAddress == 0xA4 || civAddress == 0xA2
            ? ["LSB","USB","AM","CW","CWR","RTTY","RTTYR","FM","WFM","DSTAR"]
