...
struct Ft8View: View {
    ...
    @State private var showCallGridEditor: Bool = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showCallGridEditor) {
            callGridEditor
        }
    ...
    private var toolbar: some View {
        HStack(spacing: 8) {
            ...
            Menu {
                Button("ft8_callsign_grid") {
                    callText = vm.ft8MyCall
                    gridText = vm.ft8MyGrid
                    showCallGridEditor = true
                }
    ...
    private var callGridEditor: some View {
        NavigationStack {
            Form {
                Section(header: Text("ft8_my_call")) {
                    TextField("NOCALL", text: $callText)
                        .autocapitalization(.allCharacters)
                        .disableAutocorrection(true)
                }
                Section(header: Text("ft8_my_grid")) {
                    TextField("QQ00AA", text: $gridText)
                        .autocapitalization(.allCharacters)
                        .disableAutocorrection(true)
                }
            }
            .navigationTitle(Text("ft8_callsign_grid"))
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("cancel") { showCallGridEditor = false }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button("save") {
                        vm.ft8MyCall = callText.uppercased()
                        vm.ft8MyGrid = gridText.uppercased()
                        vm.persistFt8Settings()
                        applyCallGridToWebView()
                        showCallGridEditor = false
                    }
                }
            }
        }
    ...
    private func applyCallGridToWebView() {
        let call = vm.ft8MyCall.replacingOccurrences(of: "'", with: "\\'")
        let grid = vm.ft8MyGrid.replacingOccurrences(of: "'", with: "\\'")
        let js = """
        (function(){
          localStorage.setItem('webft8-mycall','\(call)');
          localStorage.setItem('webft8-mygrid','\(grid)');
        })();
        """
...
(function() {
  ...
  window.addEventListener('DOMContentLoaded', function() {
    ...
    if ('__CALL__') {
      ...
      if (_ci) { _ci.value = '__CALL__'; _ci.dispatchEvent(new Event('input', { bubbles: true })); }
    }
    if ('__GRID__') {
      var _gi = document.getElementById('my-grid');
      if (_gi) { _gi.value = '__GRID__'; _gi.dispatchEvent(new Event('input', { bubbles: true })); }
    }
    ...
    iosBtn.addEventListener('click', function() {
      ...
      // This must happen AFTER _ft8BootstrapAudio() so _audioDest is set when
      // WebFT8 calls getUserMedia. Guard: only click when showing "Start *" (not "Stop *").
  ...
  function _patchAudioWorklet(ctx) {
    ...
    } else if (typeof ctx.audioWorklet.addModule === 'function') {
      try {
        var _origAM = ctx.audioWorklet.addModule.bind(ctx.audioWorklet);
        Object.defineProperty(ctx.audioWorklet, 'addModule', {
          value: function(url, opts) {
            return _origAM(url, opts).catch(function(e) { return Promise.resolve(); });
          },
          configurable: true, writable: true
        });
      } catch(e) {}
