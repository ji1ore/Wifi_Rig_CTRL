...
final class MainViewModel {
    ...
    private func restoreSelectionsFromSaved() {
        ...
        // Audio capture/playback: verify the saved device id still exists
        ...
        // PTT device: validate against catList (NONE always allowed)
    ...
    private func pollOnce() async {
        ...
        if useCivMode, civ.isConnected {
            ...
            if let m = s.mode { self.sharedMode = m.name; self.sharedWidth = m.filterWidth; self.cwBle.currentMode = m.name }
    ...
    func setMode(_ mode: String, width: Int) async {
        ...
        // v2.02: switch step to the per-mode preset (persisted per-mode if user later overrides).
    ...
    private func startLocalCwSidetone(text: String, wpm: Int) {
        ...
        cwSidetoneTask = Task.detached(priority: .userInteractive) {
            ...
            if !Task.isCancelled { cwBleRef.setSidetoneKeyRealtime(false) }
        }
    }
