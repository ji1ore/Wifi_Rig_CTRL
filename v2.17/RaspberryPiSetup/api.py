...
def start_rigctld(model, cat, baud, ptt="", ptt_type="RTS", release_ptt=True):
    ...
    # Use RIG (CI-V native) instead of RTS/DTR for USB serial devices (ttyACM/ttyUSB).
    # RTS/DTR toggle on USB-CDC may cause IC-705 USB audio reset; CI-V native PTT avoids this.
    # IC-7300 also requires RIG PTT when connected via USB (DTR does not key the radio).
    effective_ptt_type = ptt_type.upper()
    if effective_ptt_type in ("RTS", "DTR") and cat and ("ttyACM" in cat or "ttyUSB" in cat):
        effective_ptt_type = "RIG"
        print(f"[rigctld] USB serial detected — using -P RIG instead of -P {ptt_type.upper()}", flush=True)
...
def open_radio(model: int, cat: str, baud: int = 38400, audio: str = "", ptt: str = "", ptt_type: str = "RTS"):
    ...
    _eff = ptt_type.upper()
    if _eff in ("RTS", "DTR") and cat and ("ttyACM" in cat or "ttyUSB" in cat):
        _eff = "RIG"
