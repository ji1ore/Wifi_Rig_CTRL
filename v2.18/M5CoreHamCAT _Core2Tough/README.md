...
| ファイル | 内容 |
|---|---|
| `M5CoreHamCAT_Core2Tough_v2.18.bin` | 書き込み用の結合済みファームウェア（bootloader+partitions+アプリ本体を1本化） |
| `src/` `include/` `platformio.ini` `merge_bin.py` | ソース一式（PlatformIO プロジェクト） |

### ファームウェアの書き込み方法

**A. ソースからビルドする場合**
PlatformIO（VSCode拡張 または CLI）でこのフォルダをプロジェクトとして開き、`Upload` を実行してください。

**B. `.bin` を直接書き込む場合**（esptool.py がインストール済みであること）
```bash
esptool.py --chip esp32 --port <シリアルポート> --baud 460800 write_flash 0x0 M5CoreHamCAT_Core2Tough_v2.18.bin
```
`<シリアルポート>` は macOS なら `/dev/cu.usbserial-XXXXXXXX` のような名前です。

---

## 2. ハードウェア接続（Port の役割）

M5Stack Core2 / Tough 本体のポートは以下のように使用します。
