...
// ---- デザインパターン（配色テーマ） ----
static const int UI_THEME_COUNT = 3;
static const char *uiThemeNames[UI_THEME_COUNT] = {"OCN", "AMB", "MONO"};
...
static void getThemePalette(int theme, bool day,
...
void handleMainUIScreen()
{
  ...
  if (aprsActive)
  {
    ...
    if (aprsUseGPS && !gpsFixIsFresh(APRS_GPS_STALE_MS))
    {
      HTTPClient stopHttp;
...
// --- ロータリーエンコーダー ---
  ...
  // 物理1クリック=4エッジ変化のため、生カウントをそのまま1変化=1ステップにすると
  // 感度が高すぎる（1クリックで複数ステップ進む）。4カウント貯まるごとに1ステップとする。
  static const int ENCODER_COUNTS_PER_STEP = 2;
  int pos = encoderPos; // ← main.cpp のグローバル
  int diff = pos - lastPos;

  if (abs(diff) >= ENCODER_COUNTS_PER_STEP)
  {
    int steps = diff / ENCODER_COUNTS_PER_STEP;
    int dir = (steps > 0) ? +1 : -1;
    accumulatedDir += dir;
    lastEncoderMoveTime = millis();
    lastPos += steps * ENCODER_COUNTS_PER_STEP; // 端数は次回に持ち越す
  }
#else
  if (encoderPresent)
  {
  ...
  if (aprsTouchTarget == MENU_APRS)
  {
    ...
    if (t.wasReleased())
    {
      ...
      if (wantActive && aprsUseGPS && !gpsFixIsFresh(APRS_GPS_STALE_MS))
      {
        ...
        canvas.setTextDatum(middle_center);
        canvas.setTextColor(WHITE);
        canvas.drawString("GPS位置情報未取得", 160, 105);
        canvas.drawString("APRS開始不可", 160, 135);
      ...
      if (wantActive)
      {
        ...
        if (!started)
        {
          ...
          canvas.setTextDatum(middle_center);
          canvas.setTextColor(WHITE);
          canvas.drawString("APRS開始失敗", 160, 110);
          canvas.drawString("Pi応答なし", 160, 140);
          canvas.setTextDatum(top_left);
          canvas.pushSprite(0, 0);
          delay(1200);
        }
...
bool fetchModeList()
{
  ...
  if (useCIV)
  {
    ...
    supportedModes = {"LSB", "USB", "AM", "CW", "RTTY", "FM", "CWR", "RTTYR"};
    loadModeStepPrefs();
    applyDefaultStep(sharedMode);
    return true;
  }
