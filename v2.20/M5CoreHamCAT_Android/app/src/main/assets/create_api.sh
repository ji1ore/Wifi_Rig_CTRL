...
def _detect_vfo_mode():
    ...
    /radio/openのたびにバックグラウンドで呼ばれ、rig_vfo_modeを更新する。"""
    global rig_vfo_mode
    with rig_lock:
...
def open_radio(model: int, cat: str, baud: int = 38400, audio: str = "", ptt: str = "", ptt_type: str = "RTS"):
    global current_model, current_cat, current_baud, current_ptt, current_ptt_type, poll_started, rig_cycle_idx
    current_model = model
    current_cat = cat
    current_baud = baud
    current_ptt = ptt
...
def update_aprs_config(cfg: AprsConfig):
    ...
    # ★ ADEVICE <入力> <出力>。以前は入力=null(受信オーディオ未使用)だったため送信専用だった。
    #   受信ビーコン表示のため、同じデバイスで入出力とも有効にする(フルデュプレックス)。
    conf = (f"ADEVICE {cfg.sound_device} {cfg.sound_device}\nCHANNEL 0\n"
    ...
    def _restart_and_track_rx():
        ...
        subprocess.run(["sudo", "systemctl", "restart", "direwolf"])
        # ★ このフル設定(PTT/MYCALL込み)はそのまま受信も兼ねるため、RX専用設定への
        #   切り替えは不要。マーカーだけ揃えてから受信監視スレッドを(未起動なら)開始する。
