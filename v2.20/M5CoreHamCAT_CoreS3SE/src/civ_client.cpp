...
// ==== 音声ハードウェア(内蔵/ES8388共通、CI-V専用) ====
// 内蔵/外部を選択できないボードはModule Audio(ES8388)固定のため常にexternal扱い。
static inline bool civSpkExternal()
{
#ifdef AUDIO_SRC_SELECTABLE
  return useExternalSpk;
#else
  return true;
#endif
}
static inline bool civMicExternal()
{
#ifdef AUDIO_SRC_SELECTABLE
  return useExternalMic;
#else
  return true;
#endif
}

static void civInstallSpkI2S()
{
  i2s_config_t tx_cfg = {
  ...
  {
    civI2SReady = false;
    Serial.println("[CIV] civInstallSpkI2S: i2s_driver_install failed");
  }
}

static void civInstallMicI2S()
{
  i2s_config_t rx_cfg = {
  ...
  {
    civI2SReady = false;
    Serial.println("[CIV] civInstallMicI2S: i2s_driver_install failed");
  }
}

static void civUninstallI2S()
{
  civI2SReady = false;
...
void civDisconnect()
{
  ...
  // 音声受信用に外部SPKのI2Sを使っていた場合は解放する
  if (audioSessionReady && civSpkExternal()) civUninstallI2S();
...
String connectToCivRadio()
{
  ...
  {
    // ★ I2Sインストールを先に完了させてからcivAudioRxTaskを起動すること。
    //   逆順だと、タスク起動直後に届いた音声パケットがi2s_write()を呼んだ際、
    //   まだI2S_NUM_0がインストールされておらずNULLポインタ参照でクラッシュする
    //   (実機でGuru Meditation Error: LoadProhibitedを確認済み)。
    if (civSpkExternal()) civInstallSpkI2S();
...
void civStartTxAudio()
{
  ...
  bool micExt = civMicExternal();
  bool spkExt = civSpkExternal();

  if (civSpkHwMutex) xSemaphoreTake(civSpkHwMutex, portMAX_DELAY);

  // 本体内蔵マイク/スピーカーはI2S_NUM_1を共有し同時使用できないため、まずSPKを止める。
  if (!spkExt) M5.Speaker.end();
  // 外部SPKはI2S_NUM_0をマイクと共有するため、マイク用に明け渡す。
  if (spkExt) civUninstallI2S();

  if (!micExt) M5.Mic.begin();
  if (micExt) civInstallMicI2S();
...
void civStopTxAudio()
{
  ...
  if (micExt) civUninstallI2S(); else M5.Mic.end();
  if (spkExt) civInstallSpkI2S(); else M5.Speaker.begin();
