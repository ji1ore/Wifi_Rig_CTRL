...
│   └── res/layout/                      # XMLレイアウト
├── Wifi_RIG_CTRL_v2.31.apk              # 署名済みAPK
└── README.md
```
...
```
https://github.com/ji1ore/M5CoreHamCAT/tree/main/v2.31/M5CoreHamCAT_Android
```

1. `Wifi_RIG_CTRL_v2.31.apk` をダウンロード
...
https://github.com/ji1ore/M5CoreHamCAT/tree/main/v2.31/RaspberryPiSetup
...
- P/W/S ボタン（POW・WIDTH・SQL を1ボタンに統合）
  ...
  - 選択中の項目名をボタン下部に表示（例: P/W/S の下に "Power" / "Width" / "SQL"）
- MEM バンドメモリー機能追加
  - プリセットメモリー（160m〜70cm の代表周波数、読み取り専用）
  - ユーザーメモリー（周波数・モード・ステップを自由に登録・編集・削除）
  - 全プロファイル共通、接続先を切り替えても同じメモリーを利用可能
  - MEM ボタン短押し → 呼び出し、長押し → 管理（追加・編集・削除）

**変更**
- PTT タイプのデフォルトを CAT（RIG）に変更
  - 新規プロファイル作成時の PTT タイプ初期値を CAT(RIG) に統一
...
- モード一覧を機種ごとの動的検出に変更（`api.py`）
  - `/radio/open` 時に dump_caps でサポートモードを自動検出
  - C4FM・D-STAR 等の機種固有デジタルモードを選択可能に
  - 反映には Pi 側の更新が必要:「Update」→「Update Pi」
...
**改善**
- プロファイル自動保存（接続ボタン押下時・リグ選択時）
- APRS 設定の安定化（AP96/AP12 ビーコン途絶修正）

**修正**
- D-STAR 選択時に USB が誤送信される問題を修正（CI-V 直結モード）
...
**修正**
- APRS 受信: Mic-E パケットの東経/シンボル表示修正
