# WifiRigCTRL — Privacy Policy / プライバシーポリシー
...
WifiRigCTRL は、以下の情報のみを端末内でローカルに使用します。外部サーバー（Google・広告ネットワーク等）への送信は一切行いません。
...
WifiRigCTRL uses the following information locally on your device only. No data is transmitted to external servers (Google, ad networks, etc.).
