# WifiRigCTRL (iOS) — v2.32

iPhone / iPad からアマチュア無線機をWi-Fi経由でリモート操作するコントローラーアプリ。

> **English summary** — WifiRigCTRL is an iOS controller app for amateur (ham) radio operators. It connects to a transceiver over Wi-Fi via a Raspberry Pi (Hamlib/FastAPI) or directly to an IC-705 / IC-9700 (CI-V). Features: RX audio, PTT/mic TX, CW decode & send, BLE CW keyer, FT8/FT4, APRS beacon, multiple profiles, repeater settings (tone, offset).

---

## v2.32 での変更点

**新機能（CI-V 直接接続 — レピータ設定）**
- 周波数ディスプレイを長押しするとレピータ設定シートが開きます
- 設定可能な項目:
  - **オフセット方向**: None / + / -
  - **オフセット周波数**: プリセット（100kHz / 600kHz / 1MHz / 1.6MHz / 5MHz / 7.6MHz）またはカスタム入力
  - **トーンモード**: None / TONE / TSQL / DTCS
  - **トーン周波数**: Hz単位で指定
- 送信中はオフセット適用後の実際の TX 周波数を周波数ディスプレイに表示
- 設定はアプリ終了後も保持
...
- MEM バンドメモリー機能追加（Android版と同等）
  - プリセット / ユーザー / POTA / SOTA の4タブ構成
  - プリセットの追加・編集・削除
  - ユーザーメモリーの追加・編集・削除
  - POTA / SOTA スポットをリアルタイム取得・フィルタリング・適用
  - SSB を周波数に応じて USB/LSB に自動変換
...
- CTCSS トーン設定（None / TONE / TSQL / DTCS）
- オフセット方向（+/-）とオフセット周波数
- TX送信中はオフセット後のTX周波数を表示

### バンドメモリー（MEM）
- プリセット / ユーザー / POTA / SOTA の4タブ
- POTA/SOTA スポットをリアルタイム取得・フィルタリング
...
- ネットワーク自動スキャン（UDP ブロードキャストで Pi を自動検索）
...
**※1** CI-V 接続時: BK-IN ON → 内蔵キーヤーへ直接送信 / BK-IN OFF → PTT per element  
**※2** CI-V 接続時の BLE キーヤーも同方式。FM モードは PTT per element に自動切替。
...
│   ├── MainControlView.swift       # メインコントロール画面（RepeaterSettingsView含む）
│   ├── CwView.swift                # CW送信画面
...
│   ├── MainViewModel.swift         # メインViewModel（全状態・repeater設定管理）
...
- **Android版**: `Wifi_RIG_CTRL_ForAndroid_2.32`（本アプリのAndroid対応版）
