# Wifi RIG CTRL for Android — v2.33
...
│   └── res/layout/                      # XMLレイアウト
├── Wifi_RIG_CTRL_v2.33.apk              # 署名済みAPK
└── README.md
```
...
- WebFT8「Loading WASM で止まる」問題を修正
  - `jl1nie/webft8` の更新後に不足していた `wav-save.js` を `create_api.sh` のダウンロードリストに追加
  - `server_webft8.py`（Pi上のwebft8サーバー）を起動するたびにGitHubから最新JSファイルを自動取得するよう変更
  - FT8 WebView のブラウザキャッシュを強制クリアするよう改善

**新機能**
- Update画面に「Update WebFT8」ボタン追加（オレンジ色）
  - `server_webft8.py` のみをすばやくデプロイ（Pi再起動不要）
- 「Update Pi」完了後に「Update WebFT8」を自動実行するよう変更

**Pi側スクリプト更新**
- `create_api.sh`: `wav-save.js` をダウンロードリストに追加
- `server_webft8.py`: 起動時にGitHubから最新JSファイルを自動更新、SW.readyタイムアウトパッチ、sw.jsスキップウェイティングパッチを追加
