...
void drawPiConnectScreen()
{
    ...
    if (!useCIV)
    {
        canvas.fillRoundRect(108, 204, 80, 30, 6, canvas.color565(0, 100, 60));
        drawCentered("Scan", 148, 219, WHITE);
    }

    canvas.fillRoundRect(198, 204, 112, 30, 6, BLUE);
...
void handlePiConnectScreen()
{
    ...
    // ---- Scan (Pi モードのみ) ----
    if (!useCIV && x >= 108 && x <= 188 && y >= 204 && y <= 234)
    {
        runPiScan();
        drawScanOverlay();
        return;
