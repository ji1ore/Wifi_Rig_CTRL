...
# ALSA_CAPTURE が未設定なら USB オーディオデバイスを自動検出して .env に追記
# （非インタラクティブ: 最初に見つかった USB Audio カードを使用）
if ! grep -q "^ALSA_CAPTURE=" $ME_HOME/fastapi/.env 2>/dev/null; then
    _auto_card=$(arecord -l 2>/dev/null | awk '/USB Audio/{
        match($0, /card [0-9]+: ([^ \[,]+)/, a); if (a[1]!="") {print a[1]; exit}
    }')
    if [ -n "$_auto_card" ]; then
        ...
        echo "ALSA デバイス自動検出: $_auto_dev → .env に追記"
    else
        echo "警告: USB オーディオデバイスが未検出 — ALSA_CAPTURE は未設定のまま (デフォルト CODEC を使用)"
    fi
fi
