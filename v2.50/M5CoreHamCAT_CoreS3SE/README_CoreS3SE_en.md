# Wifi_RIG_CTRL for M5Stack CoreS3 SE  Ver 2.50

*English version of `M5CoreHamCAT_CoreS3SE/README.md`*
...
| `M5CoreHamCAT_CoreS3SE_v2.50.bin` | Pre-built firmware (merged, flashable directly at address 0x0) |

To use the pre-built `.bin` directly, flash it at address 0x0 with esptool or
similar.

```bash
esptool.py --chip esp32s3 write_flash 0x0 M5CoreHamCAT_CoreS3SE_v2.50.bin
```
...
Builds for M5Stack Core2 / M5Stack Core2 Tough are published in their own folders
(`M5CoreHamCAT_Core2` / `M5CoreHamCAT_Core2Tough`).
...
| Port | Pins | Purpose |
|---|---|---|
| **Port A** | SDA=G2 / SCL=G1 (+GND/5V) | **Rotary encoder** (connect an M5 genuine I2C encoder unit, Unit Encoder / SKU: U135) |
| **Port B** | G8 / G9 (+GND/5V) | **PTT key input (G8) + status LED (G9)**. Details below |

### Port B Wiring (Important)

- **G8 ↔ GND**: Connect the PTT contact of an external toggle switch, foot switch,
  or mechanical key (e.g., M5 genuine SK6812) here. G8 is internally pulled up
  (`INPUT_PULLUP`), so the switch only needs to be wired between G8 and Port B's GND
  (no additional pull-up resistor is required).
- **G9**: Data input for a WS2812-compatible NeoPixel (single LED). Displays TX/RX
  status by LED color (see below).

> **Important spec**: Actual transmission (PTT ON) only occurs when **both** the
> on-screen "PTT" button is ON (`txEnabled`) **and** Port B's G8 is closed (LOW).
...
The CoreS3 SE does not rotate the screen (no upside-down flip); it displays in the
normal orientation (screen flip is Core2 Tough only).

### Audio

In addition to the built-in mic/speaker, the CoreS3 SE can use an external Module
Audio (ES8388, on a separate I2C bus from Port A/B: SDA=G12, SCL=G11). On the
settings screen (PTT Method screen), you can independently select the speaker output
and mic input source (built-in or Module Audio / external).
...
| PTT | Toggle the on-screen PTT armed state ON/OFF (actual transmission also requires an external switch on Port B — see "Port B Wiring" above) |
...
### Rotary Encoder (Port A / Unit Encoder)
...
## 9. Changes in v2.50 (compared to v2.30)
...
- Stability improvements: forced termination handling for streamTask, ENOMEM
  countermeasures, etc.
- Fixed a bug where, when returning from the PTT Method screen to the rig selection
  screen, an unsaved CAT device/model selection would revert to the previously saved
  value
