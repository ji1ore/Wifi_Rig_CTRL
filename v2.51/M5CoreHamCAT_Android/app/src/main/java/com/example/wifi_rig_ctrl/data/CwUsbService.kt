...
class CwUsbService(private val context: Context) {
    ...
    var onWpmChange: ((Int) -> Unit)? = null   // M5からの0xE2 WPM通知
    ...
    // WPM for 0xE2 packet to M5 keyer
    @Volatile var keeyerWpm: Int = 20
    private var syncResponseCount = 0

    // SYNC forwarder: decouples Pi UDP round-trip from the USB read thread so that
    // slow/timeout SYNC exchanges (up to soTimeout=300ms) cannot delay key state processing.
    private val syncQueue = java.util.concurrent.LinkedBlockingQueue<ByteArray>(8)
    private var syncThread: Thread? = null

    // Scheduled sidetone: opTimeMs-based keyOn scheduling
    private val keyTimerThread: HandlerThread by lazy {
        HandlerThread("CwKeyTimer").also { it.start() }
    }
    private val keyTimerHandler: Handler by lazy { Handler(keyTimerThread.looper) }
        ...
        while (accum.isNotEmpty()) {
            ...
            when {
                b0 == 0xE0.toByte() -> {
                    ...
                    // Enqueue for async processing — keeps the read thread free for key state packets.
    ...
    private fun startSyncForwarder() {
        ...
        }.also { it.isDaemon = true; it.name = "CwUsb-SyncFwd"; it.start() }
    }

    // Forward 0xE0 SYNC from M5 Client to Pi UDP, then return 0xE1 response to M5 Client.
    // If Pi is not connected, reply with M5's own millis + 10ms so timeSynced completes
    // and the iambic keyer (and sidetone) can activate without requiring a Pi connection.
    // Send WPM to M5 keyer via 0xE2 packet
    fun sendWpmToM5(wpm: Int) {
        keeyerWpm = wpm.coerceIn(5, 99)
        if (!running) return
        val pkt = byteArrayOf(0xE2.toByte(), keeyerWpm.toByte())
        try { port?.write(pkt, 100) } catch (_: Exception) {}
    }

    fun sendPaddleSwapToM5(swapped: Boolean) {
        if (!running) return
        val pkt = byteArrayOf(0xE3.toByte(), if (swapped) 0x01.toByte() else 0x00.toByte())
        try { port?.write(pkt, 100) } catch (_: Exception) {}
    }
    ...
    private fun handleKeyState(isOn: Boolean, rawPacket: ByteArray) {
        // Immediate mode: set keyOn directly on USB read thread.
        // HandlerThread scheduling introduced ~5-10ms Android jitter which caused unstable audio.
        // USB latency (~2ms) is stable; the M5 hardware timer ensures exact unit-interval timing.
        keyOn = isOn
    ...
    private fun initSidetone() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            audioTrack?.setBufferSizeInFrames(nativeSR / 50)  // 20ms: minimal buffer to stay ~1 chunk ahead
        }

        toneThread = Thread {
            android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO)
            audioTrack?.play()
            val chunkSamples = nativeSR / 200  // 5ms chunks: reduces keyOn→tone latency
            val chunk = ShortArray(chunkSamples)
            var phase = 0.0
            val phaseInc = 2.0 * PI * SIDETONE_FREQ_HZ / nativeSR
            var envelope = 0.0
            val rampStep = 1.0 / (nativeSR * 3 / 1000)  // 3ms ramp
            // Pace writes at chunk interval so we stay ~1 chunk ahead of hardware playback.
            // Without pacing the loop fills the full 50ms buffer with stale silence,
            // delaying sidetone onset by up to one dit at 20 WPM.
            var nextWriteNs = System.nanoTime()
            while (running) {
                // Check keyOn per sample so state changes take effect within the same chunk.
                ...
                nextWriteNs += 5_000_000L  // 5ms in nanoseconds
                val sleepNs = nextWriteNs - System.nanoTime()
                if (sleepNs > 0) try {
