# Wifi_RIG_CTRL for M5Stack Core2  Ver 2.50

*English version of `M5CoreHamCAT_Core2/README.md`*
...
ICOM WLAN Remote (CI-V over WiFi), and supports APRS beacon transmission and PTT
control via Wifi_Rig_PTT (a separately sold relay board) or Hamlib.
...
| `M5CoreHamCAT_Core2_v2.50.bin` | Pre-built firmware (merged, flashable directly at address 0x0) |

To use the pre-built `.bin` directly, flash it at address 0x0 with esptool or similar.

```bash
esptool.py --chip esp32 write_flash 0x0 M5CoreHamCAT_Core2_v2.50.bin
```
...
Builds for M5Stack Core2 Tough / M5Stack CoreS3 SE are published in their own
folders (`M5CoreHamCAT_Core2Tough` / `M5CoreHamCAT_CoreS3SE`).
...
| Port | Pins | Purpose |
|---|---|---|
| **Port A** | SDA=G32 / SCL=G33 (+GND/5V) | **Rotary encoder** (connect an M5 genuine I2C encoder unit, Unit Encoder / SKU: U135) |
...
- **G13 ↔ GND**: Connect the PTT contact of an external toggle switch, foot switch,
  or mechanical key (e.g., M5 genuine SK6812) here. G13 is internally pulled up
  (`INPUT_PULLUP`), so the switch only needs to be wired between G13 and Port C's GND
...
The Core2 does not rotate the screen (no upside-down flip); it displays in the
normal orientation (screen flip is Core2 Tough only).

### Audio

In addition to the built-in mic/speaker, the Core2 can use an external Module Audio
(ES8388, I2C: SDA=G21, SCL=G22). On the settings screen (PTT Method screen), you can
independently select the speaker output and mic input source (built-in or Module
Audio / external); the default is built-in. If you don't use Module Audio, no
...
- Stability improvements: forced termination handling for streamTask, ENOMEM
  countermeasures, etc.
- The Core2 can now also switch between the built-in speaker/mic and an external
  Module Audio (ES8388) from the PTT Method screen (previously only CoreS3 SE
  supported this)
