...
void handleRigSelectScreen()
{
    ...
    // ---- ロータリーエンコーダーでリグ選択(Toughのみ物理エンコーダーを搭載) ----
#ifdef M5TOUGH
    {
        ...
        if (diff != 0)
        {
            ...
            if (total > 0)
            {
                int newSel = constrain(selRig - diff, 0, total - 1);
                if (newSel != selRig)
                {
                    selRig = newSel;
                    // 選択中の項目が見えるようスクロール位置を追従させる
                    if (selRig < rigScroll)
                        rigScroll = selRig;
                    if (selRig >= rigScroll + RIG_VISIBLE_ROWS)
                        rigScroll = selRig - RIG_VISIBLE_ROWS + 1;
                    int maxScroll = max(0, total - RIG_VISIBLE_ROWS);
                    rigScroll = constrain(rigScroll, 0, maxScroll);
                    drawRigSelectScreen();
                    return;
                }
