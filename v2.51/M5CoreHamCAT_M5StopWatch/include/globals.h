...
// ==== M5Stack Stopwatch開発キット (ESP32-S3R8, 466x466円形AMOLED) ====
// CoreS3/Core2/Toughが持っていた外付けロータリーエンコーダー・外部オーディオ
// コーデック(ES8388/Module Audio)・NeoPixel LED・GPIO足踏みPTTスイッチの
// 物理ポートは無いため、ボード別ピン定義やAUDIO_SRC_SELECTABLE分岐は廃止。
// 無線機音声はM5.Mic/M5.Speakerの内蔵デバイスのみで扱う。
#define CANVAS_SIZE 466
#define CANVAS_CENTER 233
...
extern String ssid;
extern String pass;
extern bool autoConnectTried; // 起動後、最後に接続したSSIDへの自動再接続を1回だけ試みたか
...
extern int selRig;
extern int dbgBootSavedRigId;
extern int selCat;
extern int selBaud;
extern std::vector<int> baudRates;
extern bool rigSelectFirstDraw;
extern bool rigConnectFirstDraw;
// --- CWキーヤー用USBデバイス(cw_bridge.py経由。CATデバイスとは別に選ぶ想定) ---
extern int selCwDevice;
extern bool cwBridgeOpen;
extern String cwOpenedDevice;
// --- 音声入出力デバイス(ラズパイのALSAデバイスID。/radio/audio_deviceで設定) ---
extern std::vector<String> audioDevList;
extern int selAudioDevice;
...
// --- 内蔵マイクのゲイン段階 (0-8。M5.Mic.config().magnificationへ変換して使う) ---
