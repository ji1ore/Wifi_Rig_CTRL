...
actor RigApiService {
    ...
    /// Accepts self-signed certificates (same policy as the WebView).
    func getWebft8Version() async -> String {
        let rawHost = resolvedHost.isEmpty ? host : resolvedHost
        guard !rawHost.isEmpty else { return "?" }
        // If we have a hostname (not a numeric IP), try fresh resolution to avoid mDNS issues.
        var ip = rawHost
        var test = in_addr()
        if rawHost.withCString({ inet_pton(AF_INET, $0, &test) }) != 1 {
            ip = resolveIPv4(rawHost) ?? rawHost
        }
        guard let url = URL(string: "https://\(ip):8443/server_version") else { return "?" }
        let delegate = WebFT8TrustDelegate()
        let session = URLSession(configuration: .ephemeral, delegate: delegate, delegateQueue: nil)
        var req = URLRequest(url: url, timeoutInterval: 5.0)
        req.httpMethod = "GET"
        return await withCheckedContinuation { continuation in
            session.dataTask(with: req) { data, _, error in
                _ = session  // retain session (and its delegate) until completion
                if let data = data {
                    struct Resp: Decodable { let version: String? }
                    let v = (try? JSONDecoder().decode(Resp.self, from: data))?.version ?? "?"
...
private class WebFT8TrustDelegate: NSObject, URLSessionDelegate {
