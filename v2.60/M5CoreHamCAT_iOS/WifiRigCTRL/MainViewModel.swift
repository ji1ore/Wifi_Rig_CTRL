...
final class MainViewModel {
    ...
    var webft8VersionMismatch: Bool = false
    var webft8Version: String = ""
    ...
    private func applyPendingIfConnected() {
        ...
        let (rigMode, width) = Self.mapSpotMode(spotMode, freqHz: freqHz)
        pendingFreqHz = 0
        pendingSpotMode = nil
    ...
    /// Directly queries the WebFT8 HTTPS server on port 8443 for its version.
    func fetchWebft8Version() async -> String {
        if isDemoMode { return "DEMO" }
        if await api.host.isEmpty { await adminEnsureConfigured() }
        return await api.getWebft8Version()
    }
    ...
    private func pollOnce() async {
        ...
        do {
            ...
            if let v = s.webft8_version, !v.isEmpty {
                self.webft8VersionMismatch = (v != Self.expectedWebft8Version)
                self.webft8Version = v
            }
            loggedFirstPollError = false
            consecutivePollFailures = 0
            noteActivity()
    ...
    func cwSendMorse() async -> String? {
        ...
        if sharedMode.uppercased().contains("FM") {
            cwMorseSending = true
            startLocalCwSidetone(text: text, wpm: cwWpm)
    ...
    func recallMemory(_ entry: MemoryEntry) async {
        ...
        if !entry.mode.isEmpty {
            let w = modeDefaultWidth(entry.mode)
            await setMode(entry.mode, width: w > 0 ? w : sharedWidth)
        }
        if entry.stepIndex >= 0 {
            selectedStepIndex = entry.stepIndex
    ...
    func adminUpdatePi(apiData: Data, scriptData: Data) async -> String? {
        ...
        // Wait for Pi to restart with the re-sent api.py before calling update_webft8.
        // Without this wait, update_webft8 may be handled by the old api.py (written by
        // create_api.sh) which cannot properly kill/restart the WebFT8 process.
        adminSetupLog += "\n⑤ 新しい api.py の起動待ち…"
        for attempt in 1...6 {
            try? await Task.sleep(nanoseconds: 5_000_000_000)
            adminSetupLog = "\n⑤ 起動待ち (\(attempt * 5)s)…"
        ...
        adminSetupLog += "\n⑥ WebFT8 server.py 更新中…"
        if let wErr = await adminUpdateWebFt8() {
            adminSetupLog += "\nWebFT8 更新失敗: \(wErr)"
        } else {
    ...
    func adminUpdateWebFt8() async -> String? {
        if isDemoMode { return nil }
        await adminEnsureConfigured()
        guard let url = Bundle.main.url(forResource: "server_webft8", withExtension: "py"),
              let data = try? Data(contentsOf: url) else {
            return "server_webft8.py not found in bundle"
        }
        do { try await api.sendWebFt8Update(data); return nil }
        catch { return error.localizedDescription }
    }
