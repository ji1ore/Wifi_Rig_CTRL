...
// MARK: - CI-V RS-BA1 Discovery (AreYouThere → IAmHere)

struct DiscoveredCivHost: Identifiable {
    let id = UUID()
    let ip: String
}

/// RS-BA1 AYT (type=0x03) をサブネット全体に送り IAmHere (type=0x04) 応答で IC-705 を探す。
func discoverCivHosts(port: UInt16 = 50001) async -> [DiscoveredCivHost] {
    await withCheckedContinuation { continuation in
        DispatchQueue.global(qos: .userInitiated).async {
            continuation.resume(returning: _runCivDiscovery(port: port))
        }
    }
}

private func _runCivDiscovery(port: UInt16 = 50001, timeoutSec: Double = 3.0) -> [DiscoveredCivHost] {
    var found: [DiscoveredCivHost] = []
    ...
    local.sin_len    = UInt8(MemoryLayout<sockaddr_in>.size)
    local.sin_family = sa_family_t(AF_INET)
    local.sin_port   = 0; local.sin_addr.s_addr = 0
    _ = withUnsafePointer(to: &local) {
        $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
            bind(sockfd, $0, socklen_t(MemoryLayout<sockaddr_in>.size))
        }
    ...
    // AYT packet: 16 bytes, type=0x03 (RS-BA1 AreYouThere)
    let myId = UInt32.random(in: 1..<UInt32.max)
    var ayt = [UInt8](repeating: 0, count: 16)
    ayt[0] = 0x10                                   // length LE32 = 16
    ayt[4] = 0x03                                   // type LE16 = AreYouThere
    ayt[8]  = UInt8(myId & 0xFF)
    ayt[9]  = UInt8((myId >> 8) & 0xFF)
    ayt[10] = UInt8((myId >> 16) & 0xFF)
    ayt[11] = UInt8((myId >> 24) & 0xFF)

    // ブロードキャスト + サブネット全ユニキャストへ送信
    var targets: [sockaddr_in] = [_makeSockAddrIn(ip: UInt32.max, port: port)]
    _collectIfBroadcasts(port: port, into: &targets)
    _collectSubnetUnicasts(port: port, into: &targets)
    ayt.withUnsafeBufferPointer { buf in
        for var dest in targets {
            _ = withUnsafePointer(to: &dest) {
                $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                    sendto(sockfd, buf.baseAddress, buf.count, 0, $0, socklen_t(MemoryLayout<sockaddr_in>.size))
                }
            }
        }
    }

    // IAmHere 応答を収集 (type=0x04, 16 bytes)
    ...
    while Date() < deadline {
        ...
        let n = buf.withUnsafeMutableBufferPointer { bp in
            withUnsafeMutablePointer(to: &src) { sp in
                sp.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                    recvfrom(sockfd, bp.baseAddress, bp.count, 0, $0, &srcLen)
                }
            }
        }
        if n <= 0 { usleep(20_000); continue }
        guard n == 16, buf[4] == 0x04, buf[5] == 0x00 else { continue }
        var ipBuf = [CChar](repeating: 0, count: Int(INET_ADDRSTRLEN))
        let ip: String = withUnsafePointer(to: src.sin_addr) {
            guard inet_ntop(AF_INET, $0, &ipBuf, socklen_t(INET_ADDRSTRLEN)) != nil else { return "" }
            return String(cString: ipBuf)
        }
        if !ip.isEmpty, !found.contains(where: { $0.ip == ip }) {
            found.append(DiscoveredCivHost(ip: ip))
        }
    }
    return found
}

private func _collectSubnetUnicasts(port: UInt16, into targets: inout [sockaddr_in]) {
    ...
    while let iface = cur {
        ...
        var ipBuf = [CChar](repeating: 0, count: Int(INET_ADDRSTRLEN))
        let ipStr: String = addrPtr.withMemoryRebound(to: sockaddr_in.self, capacity: 1) { sin in
            guard inet_ntop(AF_INET, &sin.pointee.sin_addr, &ipBuf, socklen_t(INET_ADDRSTRLEN)) != nil else { return "" }
            return String(cString: ipBuf)
        }
        guard !ipStr.isEmpty else { continue }
        let parts = ipStr.split(separator: ".").compactMap { Int($0) }
        guard parts.count == 4 else { continue }
        for i in 1...254 {
            if i == parts[3] { continue }
            let targetIP = "\(parts[0]).\(parts[1]).\(parts[2]).\(i)"
            let inetAddr = inet_addr(targetIP)
            guard inetAddr != INADDR_NONE else { continue }
