...
class CwBleService(private val context: Context) {
    ...
    private var keySchedThread: android.os.HandlerThread? = null
    private var keySchedHandler: android.os.Handler? = null
    @Volatile var piClockOffsetMs: Long = 0L
    private val keyOnRunnable  = Runnable { keyOn = true  }
    private val keyOffRunnable = Runnable { keyOn = false }
    ...
    fun disconnect() {
        running = false
        syncQueue.clear()
        keySchedHandler?.removeCallbacksAndMessages(null)
        keySchedThread?.quitSafely()
        keySchedThread = null
        keySchedHandler = null
        stopSidetone()
        cwAudioStream?.stop()
        cwAudioStream = null
        audioTrack?.release()
    ...
    private fun sendZeroSyncResponse() {
        ...
        // Pi未接続時はAndroid時刻(low32bit)をbytes[5-8]に入れる。
        // DualKeyはbytes[5-8]をサーバー時刻として読み取るため、
        // Piなしでも携帯時刻に同期でき opTimeMs が有効な値になる。
        val nowMs = System.currentTimeMillis()
        response[5] = ((nowMs shr 24) and 0xFF).toByte()
        response[6] = ((nowMs shr 16) and 0xFF).toByte()
        response[7] = ((nowMs shr  8) and 0xFF).toByte()
        response[8] = ( nowMs         and 0xFF).toByte()
        write(response)
        syncResponseCount++
        if (syncResponseCount == 10) sendWpmToM5(keeyerWpm)
    }
    ...
    private fun handleKeyState(isOn: Boolean, rawPacket: ByteArray) {
        // Extract opTimeMs (bytes 2-9, big-endian) and schedule sidetone at that Pi-clock target time.
        // opTimeMs ≈ Pi Unix ms (low 32-bit) when the key should fire.
        // schedDelay = fixedDelayMs − BLE_latency (automatically compensates BLE jitter).
        var opTimeMs = 0L
        for (i in 0..7) opTimeMs = (opTimeMs shl 8) or (rawPacket[2 + i].toLong() and 0xFF)
        // opTimeMsはsendZeroSyncResponse()がAndroid時刻で返すためAndroid時刻基準で比較する。
        // piClockOffsetMsを加算するとPi時刻基準になりAndroid opTimeMsとの比較が狂う。
        val schedDelay = if (opTimeMs in 2L..0xFFFFFFFFL) {
            val targetMs = opTimeMs and 0xFFFFFFFFL
            val currentMs = System.currentTimeMillis() and 0xFFFFFFFFL
            var d = targetMs - currentMs
            if (d < -1000L) d += 0x100000000L  // 32-bit wrap-around
            if (d in 0L..500L) d else 0L        // BLE遅延がfixedDelayMs超過で即時発火
        } else {
            0L
        }
        val h = keySchedHandler ?: run {
            val t = android.os.HandlerThread("CwBle-KeySched").also { it.start() }
            val handler = android.os.Handler(t.looper)
            keySchedThread = t
            keySchedHandler = handler
            handler
        }
        h.removeCallbacksAndMessages(null)
        h.postDelayed({ keyOn = isOn }, schedDelay)
