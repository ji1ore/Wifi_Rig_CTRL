...
if $_HAS_SUDO; then
    ...
    _CURRENT_USER=$ME
    for _svc in /etc/systemd/system/fastapi.service /etc/systemd/system/fastapi-audio.service /etc/systemd/system/direwolf.service; do
        if [ -f "$_svc" ]; then
            _fixed=false
    ...
    echo "$ME ALL=(ALL) NOPASSWD: /bin/systemctl restart fastapi, /bin/systemctl restart fastapi-audio, /bin/systemctl restart direwolf, /bin/systemctl start direwolf, /bin/systemctl stop direwolf, /bin/systemctl reboot, /bin/chown $ME\:$ME $ME_HOME/fastapi/api.py, /usr/bin/chown $ME\:$ME $ME_HOME/fastapi/api.py, /usr/bin/apt-get *, /usr/bin/make install, /usr/local/bin/make install, /sbin/ldconfig, /usr/sbin/ldconfig" \
...
# USB オーディオデバイスを毎回自動検出して .env に反映
# （Pi 更新後のデバイス名変化に追従するため、既存設定も上書きする）
# mawk 対応: 3引数 match() を使わず sub() で抽出
_auto_card=$(arecord -l 2>/dev/null | awk '/USB Audio/{
    sub(/.*card [0-9]+: /, ""); sub(/[ \[,].*/, ""); print; exit
}')
if [ -n "$_auto_card" ]; then
    _auto_dev="plughw:CARD=${_auto_card},DEV=0"
    grep -v "^ALSA_CAPTURE=\|^ALSA_PLAYBACK=" $ME_HOME/fastapi/.env > /tmp/_env_tmp 2>/dev/null \
        && mv /tmp/_env_tmp $ME_HOME/fastapi/.env
    echo "ALSA_CAPTURE=$_auto_dev" >> $ME_HOME/fastapi/.env
    echo "ALSA_PLAYBACK=$_auto_dev" >> $ME_HOME/fastapi/.env
    echo "ALSA デバイス自動検出・更新: $_auto_dev → .env を更新"
else
    echo "警告: USB オーディオデバイスが未検出 — ALSA_CAPTURE は現在の設定のまま"
fi
...
# ─── apt-get sudo 権限チェック ───
# sudo -n true は sudoers に true が無いと失敗するため、apt-get を直接確認する
_HAS_APT=false
if $_HAS_SUDO; then
    _HAS_APT=true
elif sudo -n /usr/bin/apt-get --version > /dev/null 2>&1; then
    _HAS_APT=true
    echo "=== apt-get NOPASSWD 確認 — パッケージインストールを実施 ==="
fi
...
# 旧 server.py はダミーとして残さない。以下の行は削除済み。
if false; then
cat << 'SRVEOF' > /dev/null
#!/usr/bin/env python3
import http.server
...
# ─── (ここまで削除済み旧 webft8 ブロック) ───

# 旧 webft8 systemd サービスが残っていれば停止・無効化
if $_HAS_SUDO && systemctl is-active webft8 > /dev/null 2>&1; then
    sudo systemctl stop webft8 2>/dev/null || true
    sudo systemctl disable webft8 2>/dev/null || true
    echo "旧 webft8.service を停止・無効化"
fi

# ─── cw_bridge.py: 独立 UDP→Serial CW ブリッジ (タイムスタンプ変換方式) ───
cat << 'CWBEOF' > $ME_HOME/cw_bridge.py
#!/usr/bin/env python3
"""M5ATOM RemoteKeyer CW ブリッジ  v2.04
...
SHIFT_FILE      = "/tmp/cw_tx_shift_hz"  # api.py が書くシフト値ファイル
...
# TX シフト VFO 制御
# ─────────────────────────────────────────────────────────────────────────────
_vfo_lock       = threading.Lock()
_vfo_saved_rx   = 0       # 0 = シフト未適用
_restore_timer  = None


def _rigctld(cmd: str) -> str:
    """rigctld (localhost:4532) にコマンドを送り応答を返す。失敗時は空文字。"""
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(2.0)
...
def _apply_vfo_shift():
    """キーON時にVFOをTX周波数へ移動。既にシフト済みなら何もしない。"""
    global _vfo_saved_rx
    with _vfo_lock:
        if _vfo_saved_rx != 0:
            return
        shift = _get_tx_shift_hz()
        if shift == 0:
            return
        freq_str = _rigctld("f")
        try:
            rx_freq = int(freq_str.split("\n")[0].strip())
        except Exception:
            return
        if rx_freq == 0:
            return
        _rigctld(f"F {rx_freq + shift}")
        _vfo_saved_rx = rx_freq
        print(f"[cw_bridge] VFO shift {rx_freq} -> {rx_freq + shift} ({shift:+d}Hz)", flush=True)


def _restore_vfo_now():
    global _vfo_saved_rx
    with _vfo_lock:
        saved = _vfo_saved_rx
        _vfo_saved_rx = 0
    if saved:
        _rigctld(f"F {saved}")
        print(f"[cw_bridge] VFO restored to {saved}", flush=True)


def _schedule_restore():
    global _restore_timer
    _cancel_restore()
    t = threading.Timer(1.0, _restore_vfo_now)
    t.daemon = True
    t.start()
    _restore_timer = t
...
def handle_key_shift(is_on: bool):
    """キー状態変化ごとに呼ぶ。ON=シフト適用、OFF=1秒後に復元。"""
    if is_on:
        _cancel_restore()
        _apply_vfo_shift()
    else:
        with _vfo_lock:
            shifted = _vfo_saved_rx != 0
        if shifted:
            _schedule_restore()
...
# ── Direwolf ADEVICE を自動修正 ───────────────────────────────
# Pi 更新・Direwolf 再インストール後もデバイスを正しく保つ
if [ -f "$ME_HOME/direwolf.conf" ]; then
    _dw_dev=$(grep "^ALSA_CAPTURE=" $ME_HOME/fastapi/.env 2>/dev/null | cut -d= -f2)
    [ -z "$_dw_dev" ] && _dw_dev="plughw:CARD=CODEC,DEV=0"
    if grep -q "^ADEVICE" "$ME_HOME/direwolf.conf"; then
        sed -i "s|^ADEVICE .*|ADEVICE null $_dw_dev|" "$ME_HOME/direwolf.conf"
    else
        sed -i "1s|^|ADEVICE null $_dw_dev\n|" "$ME_HOME/direwolf.conf"
    fi
    echo "direwolf.conf ADEVICE を自動更新: null $_dw_dev"
    if $_HAS_SUDO; then
        sudo systemctl restart direwolf && echo "direwolf 再起動完了" || true
    fi
fi
...
[dependencies]
mfsk-core = { git = "https://github.com/jl1nie/mfsk-core", tag = "v0.11.0", package = "mfsk-core" }
hound = "3"
...
fn main() {
    ...
    let wav_path = match wav_path {
        Some(p) => p,
        None => {
            eprintln!("Usage: mfsk-decode [-c MY_CALL] [-x DX_CALL] [--freq-min HZ] [--freq-max HZ] <wav>");
            std::process::exit(1);
        }
    };
...
fn load_wav_12khz(path: &str) -> Result<Vec<i16>, Box<dyn std::error::Error>> {
    ...
    let samples: Vec<i16> = match (spec.sample_format, spec.bits_per_sample) {
        ...
        (hound::SampleFormat::Float, _) => reader.samples::<f32>()
            .map(|s| (s.unwrap_or(0.0).clamp(-1.0, 1.0) * 32767.0) as i16).collect(),
        (hound::SampleFormat::Int, b) => {
            let scale = (1i64 << (b - 1)) as f64;
            reader.samples::<i32>().map(|s| ((s.unwrap_or(0) as f64 / scale) * 32767.0) as i16).collect()
        }
    };
    Ok(samples)
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < FT8_NN; i++) {
        ...
        for (int j = 0; j < samp_per_sym; j++) {
            pcm[base_i + j] = (int16_t)(sinf(phase) * 14000.0f);
            phase += dphase;
