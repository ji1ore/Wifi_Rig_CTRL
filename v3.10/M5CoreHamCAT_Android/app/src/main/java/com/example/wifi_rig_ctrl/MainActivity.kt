...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleSp2alertIntent(intent: Intent) {
        val freqHz = intent.getLongExtra(Sp2alertReceiver.EXTRA_FREQ_HZ, 0L)
        val spotMode = intent.getStringExtra(Sp2alertReceiver.EXTRA_MODE) ?: return
        if (freqHz <= 0L) return
        if (vm.isConnectedToRig.value != true) return
        val (rigMode, width) = Sp2alertReceiver.mapMode(spotMode, freqHz)
        vm.sendFreq(freqHz)
    ...
    private fun registerNetworkCallback() {
        ...
        networkCallback = object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) {
                // ConnectivityThread → post to main before accessing ViewModel (not thread-safe)
                runOnUiThread {
