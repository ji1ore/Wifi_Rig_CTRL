...
class MemoryDialogFragment : DialogFragment() {
    ...
    companion object {
        ...
        private const val ARG_FROM_FT8 = "from_ft8"
        private val BAND_ORDER = listOf(
            "160m","80m","60m","40m","30m","20m","17m","15m","12m","10m","6m","2m","70cm","Other"
        )
        fun newInstance(startTab: Int = -1, fromFt8: Boolean = false) = MemoryDialogFragment().apply {
            arguments = Bundle().apply {
                putInt(ARG_START_TAB, startTab)
                putBoolean(ARG_FROM_FT8, fromFt8)
            }
        }
    }

    private val vm: MainViewModel by activityViewModels()
    private val fromFt8: Boolean get() = arguments?.getBoolean(ARG_FROM_FT8, false) ?: false
    ...
    /** Format freq with enough decimal places to show sub-kHz precision unambiguously. */
    private fun formatSpotFreqMHz(freqHz: Long): String = when {
        freqHz % 1_000L == 0L -> "%.3f MHz".format(freqHz / 1_000_000.0)
        freqHz % 100L  == 0L  -> "%.4f MHz".format(freqHz / 1_000_000.0)
        else                  -> "%.5f MHz".format(freqHz / 1_000_000.0)
    }
    ...
    private fun updateSotaSpinners() {
        ...
        val bands = allSotaSpots.map { freqToBand(it.freqMhz) }.toSet()
            .sortedBy { b -> BAND_ORDER.indexOf(b).let { if (it < 0) 99 else it } }
        val areas = allSotaSpots.map { it.effectiveAssocCode }.filter { it.isNotEmpty() }.toSortedSet().toList()
    ...
    private fun recallAndDismiss(mem: MemoryEntry) {
        val upper = mem.mode.uppercase()
        val isFt = upper in setOf("FT8", "FT4")
        dismiss()
        if (fromFt8) {
            if (!isFt) parentFragmentManager.setFragmentResult("memory_back_to_main", android.os.Bundle())
            else vm.navigateToFt8.postValue(upper == "FT4")
        } else {
            if (isFt) {
                val b = android.os.Bundle().apply { putBoolean("isFt4", upper == "FT4") }
                parentFragmentManager.setFragmentResult("memory_ft8", b)
            }
        }
        vm.recallMemorySequenced(mem)
    }

    private fun recallAndDismiss(freqHz: Long, mode: String) {
        if (freqHz <= 0) return
        val raw = mode.uppercase()
        val normalized = if (raw.isNotEmpty() && raw != "UNKNOWN" && raw != "OTHER")
            normalizeSpotMode(raw, freqHz) else ""
        val width = if (normalized.isNotEmpty()) modeDefaultWidth(normalized) else 0
        val isFt = raw in setOf("FT8", "FT4")
        dismiss()
        if (fromFt8) {
            if (!isFt) parentFragmentManager.setFragmentResult("memory_back_to_main", android.os.Bundle())
            else vm.navigateToFt8.postValue(raw == "FT4")
        } else {
            if (isFt) {
                val b = android.os.Bundle().apply { putBoolean("isFt4", raw == "FT4") }
                parentFragmentManager.setFragmentResult("memory_ft8", b)
            }
        }
        vm.recallFreqModeSequenced(freqHz, normalized, width)
    }
    ...
    private fun fetchSota() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            var spots: List<SotaSpot> = emptyList()
            var errMsg: String? = null
            try {
                val rawSpots = SotaCluster.fetchSpots()
                spots = rawSpots
                    .groupBy { "${it.activatorCallsign ?: ""}|${it.summitCode ?: ""}" }
                    .values.map { it.maxByOrNull { s -> s.id }!! }
                if (spots.isEmpty()) errMsg = "0 spots (cluster.sota.org.uk:7300)\nPress 🔄 to retry"
            } catch (e: Exception) {
                errMsg = "Connect failed: ${e.message}\nPress 🔄 to retry"
            }
            withContext(Dispatchers.Main) {
                if (!isAdded) return@withContext
                allSotaSpots = spots
    ...
    private fun applyPotaFilter() {
        ...
        potaAdapter.update(filtered.map { spot ->
            ...
            SpotItem(
                line1  = "${spot.activator}   ${formatSpotFreqMHz(spot.freqHz)}   ${spot.mode}",
                line2  = ss2,
                line3  = buildString {
    ...
    private fun applySotaFilter() {
        ...
        val filtered = allSotaSpots.filter { spot ->
            ...
            (sotaAreaFilter.isEmpty() || spot.effectiveAssocCode == sotaAreaFilter) &&
            (q.isEmpty() || listOf(
                spot.activatorCallsign.orEmpty(), spot.displayName.orEmpty(),
                spot.summitCode.orEmpty(), spot.effectiveAssocCode, spot.comments.orEmpty()
            ).any { it.lowercase().contains(q) })
        }.let { list ->
            when (sotaSort) {
                1    -> list.sortedBy { it.activatorCallsign.orEmpty() }
                2    -> list.sortedBy { it.displayName.orEmpty() }
                else -> list.sortedBy { it.freqMhz }
            }
        }
        sotaAdapter.update(filtered.map { spot ->
            val assoc   = spot.effectiveAssocCode
            val summit  = spot.summitCode.orEmpty()
            val codeStr = listOf(assoc, summit).filter { it.isNotEmpty() }.joinToString("/")
            val l2      = "${spot.displayName.orEmpty()} ($codeStr)"
            val ss2 = SpannableString(l2)
            ...
            SpotItem(
                line1  = "${spot.activatorCallsign.orEmpty()}   ${formatSpotFreqMHz(spot.freqHz)}   ${spot.mode.orEmpty()}",
                line2  = ss2,
                line3  = buildString {
