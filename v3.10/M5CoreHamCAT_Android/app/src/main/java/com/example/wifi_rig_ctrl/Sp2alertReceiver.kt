...
class Sp2alertReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val (rawMode, width) = mapMode(spotMode, freqHz)
        val rigMode = vm.resolveMode(rawMode, freqHz)
        vm.recallFreqModeSequenced(freqHz, rigMode, width)
        val upper = spotMode.uppercase().trim()
        if (upper == "FT8" || upper == "FT4") {
            vm.navigateToFt8.postValue(upper == "FT4")
        }
    }
}
