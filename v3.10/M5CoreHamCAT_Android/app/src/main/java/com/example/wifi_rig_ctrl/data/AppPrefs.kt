...
class AppPrefs(context: Context) {
    ...
    fun applyProfile(profile: ProfileConfig) {
        p.edit()
            ...
            .putString("civConnectionType", profile.civConnectionType)
            .putString("civBtDeviceAddress", profile.civBtDeviceAddress)
            .apply()
    }
    ...
    private fun createDefaultProfile(): ProfileConfig = ProfileConfig(
        ...
        civConnectionType = p.getString("civConnectionType", "WIFI") ?: "WIFI",
        civBtDeviceAddress = p.getString("civBtDeviceAddress", "") ?: ""
    )
    ...
    var ft8AudioFreqHz: Int get() = p.getInt("ft8AudioFreqHz", 1500); set(v) = p.edit().putInt("ft8AudioFreqHz", v).apply()
    var ft8Filter: Boolean get() = p.getBoolean("ft8Filter", false); set(v) = p.edit().putBoolean("ft8Filter", v).apply()
    var ft8DecodeDepth: Int get() = p.getInt("ft8DecodeDepth", 1); set(v) = p.edit().putInt("ft8DecodeDepth", v).apply()
    var ft8AutoTxMode: String get() = p.getString("ft8AutoTxMode", "even") ?: "even"; set(v) = p.edit().putString("ft8AutoTxMode", v).apply()
    var ft8FontSize: Int get() = p.getInt("ft8FontSize", 16); set(v) = p.edit().putInt("ft8FontSize", v).apply()
    var ft8RxGain: Float get() = p.getFloat("ft8RxGain", 1.0f); set(v) = p.edit().putFloat("ft8RxGain", v).apply()
    var ft8TxGain: Float get() = p.getFloat("ft8TxGain", 0.43f); set(v) = p.edit().putFloat("ft8TxGain", v).apply()
    ...
    var civConnectionType: String get() = p.getString("civConnectionType", "WIFI") ?: "WIFI"; set(v) = p.edit().putString("civConnectionType", v).apply()
    var civBtDeviceAddress: String get() = p.getString("civBtDeviceAddress", "") ?: ""; set(v) = p.edit().putString("civBtDeviceAddress", v).apply()
    ...
    // CW TX shift persistence
    var cwTxShiftHz: Long get() = p.getLong("cwTxShiftHz", 0L); set(v) = p.edit().putLong("cwTxShiftHz", v).apply()
    var cwShiftStep: Int get() = p.getInt("cwShiftStep", 10); set(v) = p.edit().putInt("cwShiftStep", v).apply()

    // FM デジタルモード設定 (FM ボタン長押しで編集)
    ...
    companion object {
        fun defaultStepFor(mode: String): Int {
            ...
            return when {
                m.contains("FM") || m == "DSTAR" || m == "D-STAR" || m == "C4FM" -> 7  // 20kHz
                m.contains("CW") -> 2  // 100Hz
                m.contains("AM") -> 4  // 1kHz
                else -> 2  // 100Hz (SSB/PKT/RTTY等)
            }
        }
    }
}
