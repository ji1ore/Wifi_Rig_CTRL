...
class AudioTxService(baseClient: OkHttpClient) {
    ...
    @Volatile private var stoppedIntentionally = false
    ...
    fun start(url: String, apiKey: String = "") {
        ...
        val rate = Regex("rate=(\\d+)").find(url)?.groupValues?.get(1)?.toIntOrNull() ?: 8000
        ...
        txJob = CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                if (!stoppedIntentionally) Log.e("AudioTx", "TX error: ${e.message}")
                else Log.d("AudioTx", "TX ended: ${e.message}")
            }
        }
    }
