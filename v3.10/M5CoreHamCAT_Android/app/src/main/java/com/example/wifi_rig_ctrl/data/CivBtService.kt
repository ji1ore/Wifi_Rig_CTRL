...
class CivBtService {
    ...
    fun connect(device: BluetoothDevice): Boolean {
        ...
        return try {
            ...
            lastError = "BT connect failed: ${e.message}"
    ...
    fun setMode(mode: String, width: Int): Boolean {
        ...
        if (ok && (modeUpper == "PKTUSB" || modeUpper == "PKTLSB" ||
    ...
    fun setTxShift(shiftHz: Long) {
        if (!isConnected) return
        val code = when { shiftHz > 0L -> 0x11; shiftHz < 0L -> 0x10; else -> 0x00 }
        if (shiftHz != 0L) {
            var f = kotlin.math.abs(shiftHz)
            val bcd = IntArray(5) { val lo = (f % 10).toInt(); f /= 10; val hi = (f % 10).toInt(); f /= 10; (hi shl 4) or lo }
            exchange(buildCivFrame(0x0D, bcd[0], bcd[1], bcd[2], bcd[3], bcd[4]), 0xFB)
        }
        exchange(buildCivFrame(0x0F, code), 0xFB)
    ...
    fun probeSupportedModes(): List<String> {
        ...
        if (!isConnected) return getSupportedModes()
    ...
    fun pollStatus(): CivStatus? {
        ...
        val rfOut = if (tx) getRfOutputMeter() else null
        val alc   = if (tx) getAlcMeter()      else null
        return CivStatus(freq, signal, tx, mode, power, sql, bkIn, rfOut, alc)
    }
}
