...
class CivTcpService {
    ...
    /** Called on the audioRxThread for every RS-BA1 audio-data packet (raw bytes, offset, len). Used by Ft8LocalEngine in WiFi CI-V mode. */
    @Volatile var rxAudioCallback: ((ByteArray, Int, Int) -> Unit)? = null
    ...
    @Volatile private var lastDisconnectMs = 0L   // for reconnect cooldown
    ...
    // When true, CI-V commands are suppressed; only RS-BA1 audio session is maintained.
    // Used in BT CI-V hybrid mode (BT for control, RS-BA1 for audio only).
    @Volatile var audioOnly: Boolean = false
    ...
    private fun makeTxAudioPacket(audio: ByteArray, srcOffset: Int = 0, srcLen: Int = audio.size): ByteArray {
        val totalLen = AUDIO_HDR + srcLen
        val p = ByteArray(totalLen)
        ...
        putBE16(p, 22, srcLen)
        audio.copyInto(p, AUDIO_HDR, srcOffset, srcOffset + srcLen)
            ...
            when {
                ...
                r.size > AUDIO_HDR && (r[16].toInt() and 0xFF) == 0x81 -> {
                    ...
                    if (audioLen > 0) {
                        ...
                        rxAudioCallback?.invoke(r, AUDIO_HDR, audioLen)
    ...
    private fun startAudioRecordThread() {
        ...
        txAudioThread = Thread {
            ...
            val FIXED_GAIN  = 3f     // gain at micGain=1.0; typical Android mic (-20dBFS peak≈3277) → ~8k out; 8 caused tanh saturation for normal voice
    ...
    // Public entry points for BT CI-V hybrid mode: start/stop mic→RS-BA1 TX audio
    // without sending a CI-V PTT command (PTT is handled by BT RFCOMM in that mode).
    fun startTxAudioCapture() { if (isConnected) startTxAudio() }
    fun stopTxAudioCapture()  { stopTxAudio() }
    ...
    fun connect(host: String, port1: Int, port2: Int, port3: Int,
        ...
        return try {
            ...
            // Cooldown: wait until at least 2s after last disconnect so IC-705 releases the old session
            val msSinceDisc = System.currentTimeMillis() - lastDisconnectMs
            if (lastDisconnectMs > 0L && msSinceDisc < 2000L) {
                Log.i(TAG, "reconnect cooldown: waiting ${2000L - msSinceDisc}ms")
                Thread.sleep(2000L - msSinceDisc)
            }
            ...
            if (!gotIAH) { lastError = "IC-705 no response (AreYouThere 90s)"; disconnect(); return false }
            ...
            if (!gotIAR) { lastError = "IC-705 no response (IAmReady)"; disconnect(); return false }
            ...
            while (System.currentTimeMillis() < loginEnd) {
                ...
                if (err == 0xFEFFFFFFL) { lastError = "Auth failed (wrong username/password)"; disconnect(); return false }
                token = getLE32(r, 0x1c)
                Log.i(TAG, "LoginResp tok=0x${token.toString(16)}")
                gotLoginResp = true; break
            }
            if (!gotLoginResp) { lastError = "IC-705 no response (Login)"; disconnect(); return false }
            ...
            if (!gotTokenResp) { lastError = "IC-705 no response (Token)"; disconnect(); return false }
            ...
            while (System.currentTimeMillis() < rsEnd && !gotStatus) {
                ...
                when (r.size) {
                    STATUS_SIZE -> {
                        val err = getLE32(r, 0x30)
                        if (err == 0xFFFFFFFFL) { lastError = "Connect failed (Status ff)"; disconnect(); return false }
            ...
            if (!gotStatus) { lastError = "IC-705 no response (RequestStream)"; disconnect(); return false }
            ...
            // Skip in audioOnly mode: civSocket stays open for ping/pong keepalive, but we do NOT
            // register as a CI-V client (no OpenClose/transceive-enable). This prevents IC-705 from
            // seeing a second CI-V session alongside the BT RFCOMM CI-V connection (civBt), which
            // was causing IC-705 instability and dropping the RS-BA1 audio stream mid-operation.
            if (!audioOnly) {
                ...
                if (!hadKeptCivSocket) {
                    ...
                    if (!gotCivIAH) {
                        ...
                        if (civRemoteId != 0L) {
                            ...
                            lastError = "IC-705 CIV socket no response"; disconnect(); return false
                        }
                    }
                } else {
                ...
                Log.i(TAG, "audioOnly: step6 CIV handshake skipped — audio stream only, no CI-V client registration")
            }
    ...
    fun disconnect() {
        ...
        if (!seamlessReconnect) lastDisconnectMs = System.currentTimeMillis()
    }
    ...
    fun setFrequency(hz: Long): Boolean {
        ...
        if (ok) pendingFreqHz = null
        else    pendingFreqHz = hz   // exchange failed — retry on next pollStatus
        return ok
    }
    ...
    private fun parseModeBody(body: ByteArray): CivMode? {
        ...
        val dm = if (body.size > 5) body[5].toInt() and 0xFF else 0
        val name = when {
            mc == 0x01 && dm == 0x01 -> "PKTUSB"
            mc == 0x00 && dm == 0x01 -> "PKTLSB"
            mc == 0x00 -> "LSB"; mc == 0x01 -> "USB"; mc == 0x02 -> "AM"; mc == 0x03 -> "CW"
            mc == 0x04 -> "RTTY"; mc == 0x05 -> "FM"; mc == 0x06 -> "WFM"; mc == 0x07 -> "CWR"
            mc == 0x08 -> "RTTYR"; mc == 0x17 -> "DSTAR"; else -> "USB"
        }
        val w = when (mc) {
    ...
    fun setMode(mode: String, width: Int): Boolean {
        ...
        val isDataMode = modeUpper == "PKTUSB" || modeUpper == "PKTLSB"
        val mc = when (modeUpper) {
            "LSB", "PKTLSB" -> 0x00; "USB", "PKTUSB" -> 0x01; "AM" -> 0x02; "CW" -> 0x03
        ...
        if (ok) {
            // PKTUSB/PKTLSB: enable IC-705 data mode (USB-D / LSB-D).
            // USB/LSB: explicitly disable data mode when switching away from PKT modes.
            if (modeUpper == "PKTUSB" || modeUpper == "PKTLSB" ||
                modeUpper == "USB"    || modeUpper == "LSB") {
                val dm = if (isDataMode) 0x01 else 0x00
                exchange(buildCivFrame(0x1A, 0x06, dm, 0x01), 0xFB)
            }
            pendingMode = null
        }
        return ok
    ...
    /** Call after FT8 TX ends to prevent false maintenance detection while IC-705 resumes RX audio. */
    fun notifyFt8TxEnd() {
        lastAudioDataMs = System.currentTimeMillis()
    }
    ...
    fun civPttDown() {
        ...
        Log.i(TAG, "civPttDown: civStreamOpened=$civStreamOpened civAddr=0x${civAddress.toString(16)}")
        lastKnownTx = true
    ...
    // Send raw PCM audio chunk for CW-over-SSB or FT8 (caller owns pacing — not thread safe with startTxAudio)
    fun sendTxAudioDirect(pcm: ByteArray, offset: Int, len: Int) {
        if (!isConnected) return
        sendAudio(makeTxAudioPacket(pcm, offset, len))
        lastTxAudioMs = System.currentTimeMillis()  // suppress keepalive during FT8/CW TX
    }
    ...
    // ── TX meters (CI-V 0x15 0x11 = RF output, 0x15 0x12 = ALC) ──
    fun getRfOutputMeter(): Float? {
        val r = exchange(buildCivFrame(0x15, 0x11), 0x15, 0x11, 300) ?: return null
        if (r.size < 6) return null
        return bcdLevel(r[4].toInt() and 0xFF, r[5].toInt() and 0xFF) / 255f
    }

    fun getAlcMeter(): Float? {
        val r = exchange(buildCivFrame(0x15, 0x12), 0x15, 0x12, 300) ?: return null
        if (r.size < 6) return null
        return bcdLevel(r[4].toInt() and 0xFF, r[5].toInt() and 0xFF) / 255f
    }
    ...
    /** CWモードでTX周波数だけずらす (RX=VFO-A そのまま、DUP+/DUP- で TX = VFO + shiftHz) */
    fun setTxShift(shiftHz: Long) {
        if (!isConnected) return
        if (shiftHz == 0L) {
            setDuplex("")
        } else {
            setOffsetFreq(kotlin.math.abs(shiftHz))
            setDuplex(if (shiftHz > 0L) "+" else "-")
        }
    }
    ...
    fun getSupportedModes(): List<String> = when (civAddress) {
        0xA4, 0xA2 -> listOf("LSB", "USB", "AM", "CW", "CWR", "RTTY", "RTTYR", "FM", "WFM", "DSTAR", "PKTUSB", "PKTLSB")
        0x90, 0x98, 0x88, 0x70, 0x74, 0x8A -> listOf("LSB", "USB", "AM", "CW", "CWR", "RTTY", "RTTYR", "FM", "PKTUSB", "PKTLSB")
        else -> listOf("LSB", "USB", "AM", "CW", "CWR", "RTTY", "RTTYR", "FM")
    }

    // Probe actual rig capabilities. Sends read-only 0x1A 0x06 query — no rig state changes.
    fun probeSupportedModes(): List<String> {
        val base = listOf("LSB", "USB", "AM", "CW", "CWR", "RTTY", "RTTYR", "FM")
        if (!civStreamOpened) return getSupportedModes()
        val hasPkt = exchange(buildCivFrame(0x1A, 0x06), 0x1A, 0x06, 800) != null
        return if (hasPkt) base + listOf("PKTUSB", "PKTLSB") else base
    }
        ...
        return try {
            ...
            while (System.currentTimeMillis() < statusEnd) {
                ...
                if (r.size == STATUS_SIZE) {
                    ...
                    // In audioOnly mode, skip CI-V session refresh (no Step-6 OpenClose/transceive-enable).
                    // In normal mode, refresh CI-V data channel and re-enable transceive immediately.
                    if (!audioOnly) {
                        sendCiv(makeOpenClose(++civPktSeq, civSendSeqB++, true, civMyId, civRemoteId))
                        sendCiv(makeCivData(++civPktSeq, civSendSeqB++, buildCivFrame(0x16, 0x02, 0x01)))
                        lastOpenCloseMs = System.currentTimeMillis()
                    ...
                    if (spkEnabled) try { audioTrack?.play() } catch (_: Exception) {}
                    Log.i(TAG, "tryAudioOnlyReconnect: SUCCESS — audio refreshed, timer reset (audioOnly=$audioOnly)")
                    return true
                }
            }
    ...
    // Health checks for audioOnly mode (BT hybrid: BT CI-V + RS-BA1 audio).
    // Runs the same maintenance detection as pollStatus() so IC-705's periodic audio dropout
    // (maintenance cycle) is detected and recovered via tryAudioOnlyReconnect().
    // Also watches ctrl/audio pings and audioRxThread health.
    private fun pollAudioOnlyStatus() {
        ...
        if (audioRxThread?.isAlive != true) {
            Log.w(TAG, "audioOnly: audioRxThread dead → disconnect")
            isConnected = false; return
        }
        // Ctrl ping watchdog
        val cp = lastCtrlPingMs
        if (cp > 0 && now2 - cp > 5000L) {
            Log.w(TAG, "audioOnly: ctrl pings lost 5s → disconnect")
            isConnected = false; return
        }
        // Audio ping watchdog
        if (ap > 0 && now2 - ap > 5000L) {
            Log.w(TAG, "audioOnly: audio pings lost 5s → disconnect")
            isConnected = false; return
        }

        // Maintenance detection: IC-705 periodically pauses RX audio PCM.
        // Recover by resending RequestStream on the existing ctrl session.
        // Note: no lastKnownTx guard — during BT TX, IC-705 also stops RX audio.
        // tryAudioOnlyReconnect() is harmless mid-TX and audio resumes after TX.
        if (maintenanceDetectedMs != 0L ||
            (ad > 0 && now2 - ad > 3000L && ap > 0 && now2 - ap < 20_000L)) {
            if (maintenanceDetectedMs == 0L) {
                maintenanceDetectedMs = now2
                maintenanceLastRetryMs = 0L
                Log.w(TAG, "audioOnly maintenance: audio silent ${now2 - ad}ms — retrying RequestStream")
            }
            val elapsed = now2 - maintenanceDetectedMs
            val ctrlPing = lastCtrlPingMs
            if (ctrlPing > 0 && now2 - ctrlPing > 5000L) {
                Log.w(TAG, "audioOnly maintenance: ctrl pings stopped → full reconnect")
                maintenanceDetectedMs = 0L; maintenanceLastRetryMs = 0L
                seamlessReconnect = true; isConnected = false; return
            }
            if (elapsed > 90_000L) {
                Log.w(TAG, "audioOnly maintenance: 90s timeout → full reconnect")
                maintenanceDetectedMs = 0L; maintenanceLastRetryMs = 0L
                keepCivSocket = true; seamlessReconnect = true; isConnected = false; return
            }
            if (maintenanceLastRetryMs == 0L || now2 - maintenanceLastRetryMs > 3_000L) {
                maintenanceLastRetryMs = now2
                Log.i(TAG, "audioOnly maintenance: ${elapsed}ms — retrying RequestStream")
                if (tryAudioOnlyReconnect()) {
                    Log.i(TAG, "audioOnly maintenance: resumed after ${elapsed}ms")
                    maintenanceDetectedMs = 0L; maintenanceLastRetryMs = 0L
                    lastAudioDataMs = now2
                }
            }
        } else {
            maintenanceDetectedMs = 0L; maintenanceLastRetryMs = 0L
    ...
    fun pollStatus(): CivStatus? {
        ...
        if (audioOnly) { pollAudioOnlyStatus(); return null }  // audio-only: health checks only, no CI-V polling
        ...
        // TX meters: poll on every cycle during TX, skip during RX to avoid unnecessary commands
        val rfOut = if (lastKnownTx) getRfOutputMeter() else null
        val alc   = if (lastKnownTx) getAlcMeter()      else null
        return CivStatus(freq, signal, lastKnownTx, mode, power, sql, bkIn, rfOut, alc)
    }
