...
// Shared CI-V data classes (used by CivTcpService and CivBtService)
...
data class CivStatus(
    ...
    val rfOut: Float? = null, val alc: Float? = null
)
...
data class RigStatus(
    ...
    val ft8DecodeRunning: Boolean = false,
    val rfOut: Float? = null,
    val alc: Float? = null
)
...
data class PotaSpot(
    ...
    val freqHz: Long   get() = kotlin.math.round((frequency.toDoubleOrNull() ?: 0.0) * 1000).toLong()
}

// POTA hunter logbook entry (hunted park)
data class HunterPark(
    val reference: String,
    val locationDesc: String?,
    val qsos: Int = 0
)
...
data class SotaSpot(
    ...
    val freqHz: Long   get() = kotlin.math.round((frequency?.toDoubleOrNull() ?: 0.0) * 1_000_000).toLong()
    val effectiveAssocCode: String get() =
        associationCode ?: summitCode?.substringBefore("/", "") ?: ""
    val displayName: String? get() = summitDetails ?: summitName
}
...
object SotaCluster {
    ...
    fun fetchSpots(): List<SotaSpot> {
        val req  = Request.Builder()
            .url("https://api2.sota.org.uk/api/spots/-2/all/all")
            .build()
        val body = client.newCall(req).execute().use { it.body?.string() ?: return emptyList() }
        return gson.fromJson(body, object : TypeToken<List<SotaSpot>>() {}.type) ?: emptyList()
    }
}