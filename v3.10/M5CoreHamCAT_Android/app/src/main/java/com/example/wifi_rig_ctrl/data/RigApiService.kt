...
class RigApiService(private var hostName: String, private var apiPort: Int, private var apiKey: String = "") {
    ...
    // SSE専用: readTimeoutを0(無限)にしてkeepAliveで接続を維持
    internal val sseClient = OkHttpClient.Builder()
    ...
    // FT8 TX専用: 長いread timeout（最大30s待機+12s送信）、cachingDnsでmDNS解決をキャッシュ共有
    ...
    fun getStatus(): RigStatus? {
        ...
        return try {
            ...
            val rfOut = (doc["rf_out"] as? Double)?.toFloat()
            val alc   = (doc["alc"]    as? Double)?.toFloat()
            RigStatus(
                freq = freq,
                ...
                ft8DecodeRunning = doc["ft8_decode_running"] as? Boolean ?: false,
                rfOut = rfOut,
                alc = alc
            )
        } catch (_: Exception) { null }
    ...
    fun setTxShift(shiftHz: Long) = post("/radio/set_tx_shift",
        FormBody.Builder().add("shift_hz", shiftHz.toString()).build())
    ...
    fun ft8Tx(msg: String, audioFreqHz: Int = 1500, rate: Int = 12000, isFt4: Boolean = false, txMode: String = ""): Boolean = try {
        val json = """{"msg":${com.google.gson.JsonPrimitive(msg)},"audio_freq":$audioFreqHz,"rate":$rate,"is_ft4":$isFt4,"tx_mode":"$txMode"}"""
    ...
    fun ft8TxResult(msg: String, audioFreqHz: Int = 1500, rate: Int = 12000, isFt4: Boolean = false, txMode: String = ""): Pair<Boolean, String> = try {
        val json = """{"msg":${com.google.gson.JsonPrimitive(msg)},"audio_freq":$audioFreqHz,"rate":$rate,"is_ft4":$isFt4,"tx_mode":"$txMode"}"""
        val req = Request.Builder().url("$baseUrl/radio/ft8_tx").withApiKey()
            .post(json.toRequestBody("application/json".toMediaType())).build()
        ft8TxClient.newCall(req).execute().use { r ->
            val body = r.body?.string() ?: ""
            if (r.isSuccessful) Pair(true, "OK")
            else Pair(false, "HTTP ${r.code}: $body")
        }
    ...
    fun ft8Start(): Boolean = try {
        val req = Request.Builder().url("$baseUrl/ft8/start").withApiKey()
            .post("".toRequestBody("text/plain".toMediaType())).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8Stop(): Boolean = try {
        val req = Request.Builder().url("$baseUrl/ft8/stop").withApiKey()
            .post("".toRequestBody("text/plain".toMediaType())).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8RxMsgsUrl(): String {
        val keyParam = if (apiKey.isNotEmpty()) "?api_key=$apiKey" else ""
        return "$baseUrl/ft8/rx_msgs$keyParam"
    }

    fun ft8SpectrumUrl(): String {
        val keyParam = if (apiKey.isNotEmpty()) "?api_key=$apiKey" else ""
        return "$baseUrl/ft8/spectrum$keyParam"
    }

    fun ft8DecodeDebug(): String = try {
        val req = Request.Builder().url("$baseUrl/ft8/decode_debug").withApiKey().get().build()
        client.newCall(req).execute().use { r -> r.body?.string() ?: "no body" }
    } catch (e: Exception) { "error: ${e.message}" }

    fun ft8SetDepth(depth: Int): Boolean = try {
        val req = Request.Builder().url("$baseUrl/ft8/set_depth?depth=$depth").withApiKey()
            .post("".toRequestBody("text/plain".toMediaType())).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8SetFt4(isFt4: Boolean): Boolean = try {
        val req = Request.Builder().url("$baseUrl/ft8/set_ft4?is_ft4=$isFt4").withApiKey()
            .post("".toRequestBody("text/plain".toMediaType())).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8SetTxGain(gain: Float): Boolean = try {
        val req = Request.Builder().url("$baseUrl/ft8/set_tx_gain?gain=$gain").withApiKey()
            .post("".toRequestBody("text/plain".toMediaType())).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8SetAutoTx(active: Boolean, mode: String, msg: String, audioFreqHz: Int): Boolean = try {
        val json = """{"active":$active,"mode":"$mode","msg":${com.google.gson.JsonPrimitive(msg)},"audio_freq":$audioFreqHz}"""
        val body = json.toRequestBody("application/json".toMediaType())
        val req = Request.Builder().url("$baseUrl/ft8/auto_tx").withApiKey().post(body).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8CqAuto(active: Boolean, msg: String, mode: String, audioFreqHz: Int,
                  myCall: String, myGrid: String): Boolean = try {
        fun esc(s: String) = com.google.gson.JsonPrimitive(s).toString()
        val json = """{"active":$active,"msg":${esc(msg)},"mode":"$mode","audio_freq":$audioFreqHz,"my_call":${esc(myCall)},"my_grid":${esc(myGrid)}}"""
        val body = json.toRequestBody("application/json".toMediaType())
        val req = Request.Builder().url("$baseUrl/ft8/cq_auto").withApiKey().post(body).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8StartQso(
        firstMsg: String, dxCall: String, myCall: String, txMode: String,
        audioFreqHz: Int, initialState: Int = 0,
        stepSnrMsg: String = "", step2Msg: String = "", step3Msg: String = "", step4Msg: String = ""
    ): Boolean = try {
        fun esc(s: String) = "\"${s.replace("\\", "\\\\").replace("\"", "\\\"")}\""
        val json = """{"first_msg":${esc(firstMsg)},"dx_call":${esc(dxCall)},"my_call":${esc(myCall)},"tx_mode":"$txMode","audio_freq":$audioFreqHz,"initial_state":$initialState,"step_snr_msg":${esc(stepSnrMsg)},"step2_msg":${esc(step2Msg)},"step3_msg":${esc(step3Msg)},"step4_msg":${esc(step4Msg)}}"""
        val body = json.toRequestBody("application/json".toMediaType())
        val req = Request.Builder().url("$baseUrl/ft8/start_qso").withApiKey().post(body).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8SetDecodeFilter(centerFreqs: List<Int>, rangeHz: Int = 60): Boolean = try {
        val freqJson = centerFreqs.joinToString(",", "[", "]")
        val json = """{"freqs":$freqJson,"range":$rangeHz}"""
        val body = json.toRequestBody("application/json".toMediaType())
        val req = Request.Builder().url("$baseUrl/ft8/set_decode_filter").withApiKey().post(body).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }

    fun ft8CancelQso(): Boolean = try {
        val body = "{}".toRequestBody("application/json".toMediaType())
        val req = Request.Builder().url("$baseUrl/ft8/cancel_qso").withApiKey().post(body).build()
        client.newCall(req).execute().use { it.isSuccessful }
    } catch (_: Exception) { false }
    ...
    fun triggerMfskBuild(force: Boolean = false): String? = try {
        val url = if (force) "$baseUrl/admin/build_mfsk?force=true" else "$baseUrl/admin/build_mfsk"
        val req = Request.Builder().url(url).withApiKey()
    ...
    fun getMfskBuildLog(): Pair<Boolean, String> = try {
        val req = Request.Builder().url("$baseUrl/admin/mfsk_build_log").withApiKey().get().build()
        val logClient = OkHttpClient.Builder()
            .connectTimeout(5, TimeUnit.SECONDS)
            .readTimeout(10, TimeUnit.SECONDS)
            .build()
