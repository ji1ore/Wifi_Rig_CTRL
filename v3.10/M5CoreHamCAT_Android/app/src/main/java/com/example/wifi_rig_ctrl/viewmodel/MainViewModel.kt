...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    val hunterStore = HunterStore(app)
    ...
    @Volatile private var civSimplexResetSent = false
    ...
    val sharedRfOut = MutableLiveData(0f)
    val sharedAlc = MutableLiveData(0f)

    // SP2ALERT/MEM から FT8 画面へのナビゲーションイベント (Boolean = isFt4)
    val navigateToFt8 = MutableLiveData<Boolean?>()
    ...
    val cwTxShiftHz = MutableLiveData(prefs.cwTxShiftHz)

    fun setCwTxShift(shiftHz: Long) {
        val rounded = (shiftHz / 10L) * 10L
        cwTxShiftHz.value = rounded
        prefs.cwTxShiftHz = rounded
        if (useCIV.value == true && isCivConnected) {
            viewModelScope.launch(Dispatchers.IO) {
                if (isCivBt) civBt.setTxShift(0L) else civ.setTxShift(0L)
            }
        } else if (useCIV.value != true) {
            viewModelScope.launch(Dispatchers.IO) { api.setTxShift(rounded) }
        }
    }

    fun applySavedCwShift() {
        val saved = prefs.cwTxShiftHz
        if (saved != 0L) setCwTxShift(saved)
    }
    ...
    companion object {
        ...
        const val EXPECTED_PI_API_VERSION = "3.10"
    ...
    suspend fun connectToRasPi(): String? = withContext(Dispatchers.IO) {
        civSimplexResetSent = false
        // CI-V direct connection path
        if (useCIV.value == true) {
            val addr = civAddress.value ?: 0xA4
            if (isCivBt) {
                // Bluetooth CI-V path
                val btAddr = civBtDeviceAddress.value ?: return@withContext "BT device not set"
                ...
                } ?: return@withContext "BT device not found: $btAddr"
                savedBtDevice = device
                civBt.civAddress = addr
                if (!civBt.connect(device)) return@withContext "BT CI-V connection failed\n${civBt.lastError}"
                ...
                if (audioHost.isNotEmpty()) {
                    ...
                    if (!audioOk) {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            errorMessage.value = "RS-BA1 Audio connect failed (BT OK): ${civ.lastError}"
                        }
                    } else {
                ...
                if (!civ.connect(host, port, port2, port3, user, pass, wifiNet)) return@withContext "CI-V connection failed\n${civ.lastError}"
    ...
    private suspend fun updateCivStatus() {
        val st = withContext(Dispatchers.IO) {
            if (isCivBt) {
                // BT hybrid: also run audio watchdog on civ so maintenance recovery fires
                if (civ.audioOnly) {
                    if (civ.isConnected) {
                        civ.pollStatus()  // runs pollAudioOnlyStatus() — maintenance detection + watchdogs
                    } else {
                        // civ dropped (audio watchdog triggered disconnect) — reconnect RS-BA1 audio
                        val host = civHost.value ?: ""
                        if (host.isNotEmpty()) {
                            val now = System.currentTimeMillis()
                            if (now - lastCivReconnectAttemptMs >= 30_000L) {
                                ...
                                civ.audioOnly = true
                                val ok = civ.connect(host, port, port2, port3, user, pass, wifiNet)
                                if (!ok) { civ.audioOnly = false; android.util.Log.w("MainVM", "BT audio reconnect failed: ${civ.lastError}") }
                            }
                        }
                    }
                }
            ...
            if (!isCivConnected) {
                ...
                lastCivReconnectAttemptMs = now
                if (isCivBt) {
                    val device = savedBtDevice ?: run { errorMessage.postValue("BT CI-V disconnected"); return }
                    val ok = withContext(Dispatchers.IO) { civBt.connect(device) }
                    if (!ok) errorMessage.postValue("BT CI-V reconnect failed: ${civBt.lastError}")
                } else {
                    val host = civHost.value ?: run { errorMessage.postValue("CI-V disconnected"); return }
                    val port  = civPort.value  ?: 50001
                    val port2 = civPort2.value ?: 50002
                    val port3 = civPort3.value ?: 50003
                    val user  = civUser.value  ?: ""
                    ...
                    if (!ok) errorMessage.postValue("CI-V reconnect failed: ${civ.lastError}")
                }
            }
            return
        }
        // On first successful CI-V poll, ensure DUP is Simplex (stream is confirmed open here)
        if (!civSimplexResetSent) {
            civSimplexResetSent = true
        ...
        sharedRfOut.value = st.rfOut ?: if (!st.tx) 0f else sharedRfOut.value
        sharedAlc.value  = st.alc   ?: if (!st.tx) 0f else sharedAlc.value
    }
    ...
    fun resolveMode(mode: String, freqHz: Long): String {
        if (mode.equals("SSB", ignoreCase = true) &&
            (supportedModes.value ?: emptyList()).none { it.equals("SSB", ignoreCase = true) }) {
            return if (freqHz >= 10_000_000L) "USB" else "LSB"
        }
        return mode
    }

    fun modeDefaultWidth(mode: String): Int = when {
        mode == "FM" || mode == "PKTFM" -> 12000
        mode == "FMN"                   -> 9000
        mode.startsWith("CW")           -> prefs.defaultCwWidth
        mode == "USB" || mode == "LSB"  -> prefs.defaultSsbWidth
        else                            -> 0
    }

    fun recallMemorySequenced(mem: MemoryEntry) {
        val targetFreq = mem.freqHz
        val mode = resolveMode(mem.mode, targetFreq)
        val width = modeDefaultWidth(mode)
        val stepIdx = if (mem.stepIndex >= 0) mem.stepIndex
                      else prefs.getModeStep(mode.ifEmpty { sharedMode.value ?: "" })
        viewModelScope.launch {
            ...
            if (mode.isNotEmpty()) {
                ...
                selectedStep.value = stepIdx
            }
            setRepeater(mem.toneMode.orEmpty(), mem.toneHz ?: 0.0, mem.offsetDir.orEmpty(), mem.offsetHz ?: 0L)
            repeat(3) {
                delay(150L)
                if (mode.isNotEmpty()) awaitMode(mode, width)
                awaitFreq(targetFreq)
            }
        }
    }

    fun recallFreqModeSequenced(freqHz: Long, mode: String, width: Int) {
        val stepIdx = prefs.getModeStep(mode.ifEmpty { sharedMode.value ?: "" })
        viewModelScope.launch {
            delay(100L)
            if (mode.isNotEmpty()) {
                repeat(2) {
    ...
    // モード先行・周波数後送で確実に順序を保証するsuspend版
    private suspend fun awaitMode(mode: String, width: Int) {
        ...
        val modeToSend = if (mode.uppercase() == "USB-D") "PKTUSB" else mode
        val usesCIV = useCIV.value == true
        val bt = isCivBt
        withContext(Dispatchers.IO) {
            if (usesCIV) { if (bt) civBt.setMode(modeToSend, width) else civ.setMode(modeToSend, width) }
            else api.setMode(modeToSend, width)
        }
    }

    private suspend fun awaitFreq(freqHz: Long) {
        ...
        val bt = isCivBt
        withContext(Dispatchers.IO) {
            if (usesCIV) { if (bt) civBt.setFrequency(freqHz) else civ.setFrequency(freqHz) }
            else api.setFreq(freqHz)
        }
    }
    ...
    fun sendMode(mode: String, width: Int) {
        ...
        if (useCIV.value == true) {
            viewModelScope.launch(Dispatchers.IO) {
                if (isCivBt) civBt.setMode(modeToSend, width) else civ.setMode(modeToSend, width)
            }
        } else {
            viewModelScope.launch(Dispatchers.IO) { api.setMode(modeToSend, width) }
        }
    }
    ...
    fun civSetSpk(on: Boolean) {
        ...
        if (on) {
            if (!civ.audioOnly || !civ.isConnected) {
                spkEnabled.value = false  // revert UI state so button doesn't show ON with no audio
                errorMessage.value = "BT CI-V audio: Enter RS-BA1 Audio Host on Connect screen and reconnect"
                return
            }
            civ.spkEnabled = true
        } else {
    ...
    suspend fun quickUpdatePiApi(): String? = withContext(Dispatchers.IO) {
        try {
            val apiBytes = getApplication<Application>().assets.open("api.py").readBytes()
            api.sendApiUpdate(apiBytes)
    ...
    suspend fun updatePiSoftware(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            // Poll setup_log until create_api.sh finishes ("=== DONE ===").
            // Marker is ASCII to avoid JSON Unicode-escape decode issues in the regex parser.
            // Timeout is 8 min — heredoc writes on Pi Zero SD card can take several minutes.
            // Note: do NOT re-send api.py here — create_api.sh already restores api.py.bak2
            // (the version we just sent). Re-sending triggers _restart() in FastAPI which kills
            // the reboot subprocess (same cgroup), causing the subsequent reboot to fail silently.
            val setupDeadline = System.currentTimeMillis() + 480_000L
            var setupFinished = false
            while (System.currentTimeMillis() < setupDeadline) {
                Thread.sleep(3000)
                val (_, log) = try { api.getSetupLog() } catch (e: Exception) { false to "" }
                if (log.contains("=== DONE ===")) { setupFinished = true; break }
            }

            if (!setupFinished) return@withContext "setup timeout. Restart Pi."
    ...
    suspend fun rebootPiAndWait(): String = withContext(Dispatchers.IO) {
        ...
        // Pi Zero: reboot cmd runs after 2s sleep; shutdown takes 10-20s total.
        // Wait for Pi to go OFFLINE first, then wait for it to come back up.
        val offlineDeadline = System.currentTimeMillis() + 45_000L
        var wentOffline = false
        while (System.currentTimeMillis() < offlineDeadline) {
            Thread.sleep(3000)
            if (api.getStatus() == null) { wentOffline = true; break }
        }
    ...
    suspend fun triggerMfskBuildAfterUpdate(): String = withContext(Dispatchers.IO) {
        // Pi Zero の起動は遅い。FastAPI が安定するまで最大60秒待つ。
        for (attempt in 1..20) {
            Thread.sleep(3000)
            val err = try { api.triggerMfskBuild(force = true) } catch (e: Exception) { e.message }
            if (err == null) {
                // ワーカーがログを書き始めるまで最大15秒待つ
                val deadline = System.currentTimeMillis() + 15_000L
                while (System.currentTimeMillis() < deadline) {
                    Thread.sleep(2000)
                    val (running, log) = try { api.getMfskBuildLog() } catch (_: Exception) { false to "" }
                    if (running || log.contains("ビルド開始")) return@withContext "mfsk build started"
                }
                return@withContext "mfsk build started (log pending)"
            }
        }
        "mfsk build trigger failed (Pi unreachable?)"
    }
    ...
    suspend fun syncPiTime(): String = withContext(Dispatchers.IO) {
        ...
        val r = try { api.syncTime(nowMs) } catch (e: Exception) { return@withContext "Sync failed: ${e.message}" }
        if (!r.ok) return@withContext "Sync failed"
        val piSource = when {
        ...
        val preStr = preOffsetMs?.let {
            ...
            " (pre-sync offset ${s}ms)"
        } ?: ""
        "Sync OK[$piSource]$preStr"
    }
    ...
    private fun stopCwServerPolling() {
        ...
        cwServerConnected.postValue(false)
        cwServerSynced.postValue(false)
    }
                ...
                if (isCwMode) {
                    ...
                    if (isCivBt) {
                        if (civ.audioOnly && civ.isConnected) {
                            ...
                            withContext(Dispatchers.Main) {
                                errorMessage.value = "BT CI-V: Switch to CW mode or set RS-BA1 Audio Host"
                            }
                        }
                    } else {
    ...
    fun disconnectFromRig() {
        civSimplexResetSent = false
        btHybridAudioReady.postValue(false)
        if (useCIV.value == true) {
            if (isCivBt) {
                civBt.disconnect()
        ...
        isDemoMode.postValue(false)
        externalPttActive = false
        cwCqRepeatJob?.cancel(); cwCqRepeatJob = null
        cwCqRepeating.postValue(false); cwCqRepeatStatus.postValue("")
        cwTextJob?.cancel(); cwTextJob = null
        cwTxBusy.postValue(false)
        serverWatchdogJob?.cancel()
        serverWatchdogJob = null
        pttOnJob?.cancel()
        pttOnJob = null
        ...
        isConnectedToRig.postValue(false)
        txEnabled.postValue(false)
        spkEnabled.postValue(false)
        aprsActive.postValue(false)
        cwUsbConnected.postValue(false)
        cwDecoding.postValue(false)
    }
