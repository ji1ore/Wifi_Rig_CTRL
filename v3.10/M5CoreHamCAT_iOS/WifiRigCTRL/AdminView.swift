...
struct AdminView: View {
    ...
    var body: some View {
        Form {
            ...
            Section(header: Label("mfsk-decode Build Log", systemImage: "doc.text.magnifyingglass")
                        .foregroundStyle(.purple)) {
                HStack {
                    statusBadge(vm.adminMfskRunning)
                    Spacer()
                    Button {
                        Task { await vm.adminPollMfskLog() }
                    } label: {
                        Label("Refresh", systemImage: "arrow.clockwise")
                    }
                    .tint(.purple)
                }
                logView(vm.adminMfskLog)
            }
    ...
    private func runUpdate() async {
        ...
        if let shURL = Bundle.main.url(forResource: "create_api", withExtension: "sh"),
           let shData = try? Data(contentsOf: shURL) {
            ...
            // Update Pi 完了 → Pi リブート → mfsk-decode ビルドトリガー
            localMessage = "Update complete. Rebooting Pi..."
            let rebootResult = await vm.rebootPiAndWait()
            localMessage = "\(rebootResult) — Starting mfsk build..."
            let mfskResult = await vm.triggerMfskBuildAfterUpdate()
            localMessage = "OK: \(rebootResult) — \(mfskResult)"
            await vm.adminPollMfskLog()
        } else {
