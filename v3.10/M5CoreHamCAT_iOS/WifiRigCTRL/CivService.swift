...
final class CivService: @unchecked Sendable {
    ...
    // MARK: - FT8 local engine hook (set by Ft8LocalEngine when in CI-V WiFi mode)
    var rxAudioCallback: ((Data) -> Void)?
    ...
    private func parseMode(_ body: Data) -> CivMode? {
        ...
        let dm = body.count > 5 ? Int(body[5]) : 0
        let name: String
        if mc == 0x01 && dm == 0x01 { name = "PKTUSB" }
        else if mc == 0x00 && dm == 0x01 { name = "PKTLSB" }
        else { name = switch mc {
    ...
    func setMode(_ mode: String, width: Int) -> Bool {
        ...
        let isDataMode = modeUpper == "PKTUSB" || modeUpper == "PKTLSB"
        let mc: Int = switch modeUpper {
            case "LSB", "PKTLSB": 0x00; case "USB", "PKTUSB": 0x01
            case "AM": 0x02; case "CW": 0x03; case "RTTY": 0x04; case "FM": 0x05
            case "WFM": 0x06; case "CWR","CW-R": 0x07; case "RTTYR": 0x08
        ...
        if ok {
            if isDataMode || modeUpper == "USB" || modeUpper == "LSB" {
                let dm = isDataMode ? 1 : 0
                _ = exchange(buildCivFrame(0x1A, 0x06, dm, 0x01), cmd: 0xFB)
            }
            pendingMode = nil
        }
        return ok
    ...
    func getRfOutputMeter() -> Float? {
        guard let r = exchange(buildCivFrame(0x15, 0x11), cmd: 0x15, subCmd: 0x11, timeoutMs: 300),
              r.count >= 6 else { return nil }
        return Float(bcdLevel(r[4], r[5])) / 255
    }

    func getAlcMeter() -> Float? {
        guard let r = exchange(buildCivFrame(0x15, 0x12), cmd: 0x15, subCmd: 0x12, timeoutMs: 300),
              r.count >= 6 else { return nil }
        return Float(bcdLevel(r[4], r[5])) / 255
    }
    ...
    @discardableResult
    func setTxShift(_ shiftHz: Int64) -> Bool {
        guard isConnected else { return false }
        if shiftHz == 0 {
            return exchange(buildCivFrame(0x0F, 0x00), cmd: 0xFB) != nil
        }
        var f = abs(shiftHz)
        let bcd = (0..<5).map { _ -> Int in let lo = Int(f % 10); f /= 10; let hi = Int(f % 10); f /= 10; return (hi << 4) | lo }
        _ = exchange(buildCivFrame(0x0D, bcd[0], bcd[1], bcd[2], bcd[3], bcd[4]), cmd: 0xFB)
        let code = shiftHz > 0 ? 0x11 : 0x10
        return exchange(buildCivFrame(0x0F, code), cmd: 0xFB) != nil
    }
    ...
    private static let modeTable: [Int: [String]] = [
        0xA4: ["LSB","USB","AM","CW","CWR","RTTY","RTTYR","FM","WFM","DSTAR","PKTUSB","PKTLSB"], // IC-705
        0xA2: ["LSB","USB","AM","CW","CWR","RTTY","RTTYR","FM","WFM","DSTAR","PKTUSB","PKTLSB"], // IC-9700
        0x88: ["LSB","USB","AM","CW","CWR","RTTY","RTTYR","FM","DSTAR","PKTUSB","PKTLSB"],        // IC-7100
        0x90: ["LSB","USB","AM","CW","CWR","RTTY","RTTYR","FM","PKTUSB","PKTLSB"],                // IC-7300
        0x98: ["LSB","USB","AM","CW","CWR","RTTY","RTTYR","FM","PKTUSB","PKTLSB"],                // IC-7610
    ]
    ...
    func probeSupportedModes() -> [String] {
        let base = ["LSB", "USB", "AM", "CW", "CWR", "RTTY", "RTTYR", "FM"]
        guard civStreamOpened else { return getSupportedModes() }
        let hasPkt = exchange(buildCivFrame(0x1A, 0x06), cmd: 0x1A, subCmd: 0x06, timeoutMs: 800) != nil
        return hasPkt ? base + ["PKTUSB", "PKTLSB"] : base
    }
    ...
    func pollStatus() -> CivStatus? {
        ...
        let rfOut = lastKnownTx ? getRfOutputMeter() : nil
        let alc   = lastKnownTx ? getAlcMeter()      : nil
