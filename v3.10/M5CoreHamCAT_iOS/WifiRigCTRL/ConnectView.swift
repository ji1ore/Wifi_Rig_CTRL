...
struct ConnectView: View {
    ...
    @State private var ssbWidthText: String = ""
    @State private var cwWidthText:  String = ""
    @State private var localError: String?
    @State private var showSaveDialog: Bool = false
    @State private var showPotaHuntLog: Bool = false
    ...
    @State private var isScanningCiv: Bool = false
    @State private var discoveredCivHosts: [DiscoveredCivHost] = []
    @State private var showCivDiscoveryResults: Bool = false
    @State private var showNoCivHostsAlert: Bool = false
    ...
    var body: some View {
        Form {
                ...
                Section(header: Label("Saved Profiles", systemImage: "person.crop.circle.fill").foregroundStyle(.indigo)) {
                    ForEach(vm.profiles) { p in
                        Button {
                            ...
                            HStack {
                                Image(systemName: p.useCivMode
                                      ? "antenna.radiowaves.left.and.right.circle.fill"
                                      : "server.rack")
                                    .foregroundStyle(p.useCivMode ? .teal : .indigo)
                                    .frame(width: 24)
            ...
            Section(header: Label("Station", systemImage: "person.wave.2.fill").foregroundStyle(.indigo)) {
                ...
                LabeledContent("SSB Width (Hz)") {
                    TextField("3000", text: $ssbWidthText)
                        .multilineTextAlignment(.trailing)
                        .keyboardType(.numberPad)
                        .frame(maxWidth: 80)
                        .onChange(of: ssbWidthText) { _, v in
                            if let n = Int(v), n >= 100 { vm.defaultSsbWidth = min(n, 10000) }
                        }
                }
                LabeledContent("CW Width (Hz)") {
                    TextField("500", text: $cwWidthText)
                        .multilineTextAlignment(.trailing)
                        .keyboardType(.numberPad)
                        .frame(maxWidth: 80)
                        .onChange(of: cwWidthText) { _, v in
                            if let n = Int(v), n >= 50 { vm.defaultCwWidth = min(n, 3000) }
                        }
                }
                if !vm.useCivMode {
            ...
            if vm.useCivMode {
                ...
                Section {
                    ...
                    Button {
                        newProfileName = vm.civHost.isEmpty ? "Profile \(vm.profiles.count + 1)" : vm.civHost
                        showSaveDialog = true
                    } label: {
                        Label("Save as Profile…", systemImage: "square.and.arrow.down")
                    }
            ...
            // MARK: POTA Hunt Log (v3.01)
            Section(header: Label("POTA", systemImage: "map.fill").foregroundStyle(.green)) {
                Button {
                    showPotaHuntLog = true
                } label: {
                    HStack {
                        Label("POTA ハント履歴取り込み", systemImage: "arrow.down.circle.fill")
                            .foregroundStyle(.green)
                        Spacer()
                        let count = vm.hunterStore.parkCount
                        if count > 0 {
                            Text("\(count) parks")
        ...
        .onAppear {
            ...
            ssbWidthText  = String(vm.defaultSsbWidth)
            cwWidthText   = String(vm.defaultCwWidth)
        ...
        .confirmationDialog("IC-705を選択", isPresented: $showCivDiscoveryResults, titleVisibility: .visible) {
            ForEach(discoveredCivHosts) { host in
                Button(host.ip) {
                    vm.civHost = host.ip
                }
            }
        ...
        .alert("IC-705が見つかりません", isPresented: $showNoCivHostsAlert) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("RS-BA1サーバーが見つかりませんでした。\nIC-705がWLANに接続されてRS-BA1が有効であることを確認してください。")
        }
        .sheet(isPresented: $showPotaHuntLog) {
            PotaHuntLogView { json in
                vm.hunterStore.importFromApiJson(json)
            }
        }
    ...
    private func scanCivHosts() async {
        isScanningCiv = true
        let hosts = await discoverCivHosts()
        isScanningCiv = false
        if hosts.isEmpty {
            showNoCivHostsAlert = true
        } else {
            discoveredCivHosts = hosts
            showCivDiscoveryResults = true
        }
    }
}
