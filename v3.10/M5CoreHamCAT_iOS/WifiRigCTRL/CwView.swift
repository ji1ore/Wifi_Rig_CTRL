...
struct CwView: View {
    ...
    private var portraitView: some View {
        ScrollView {
            VStack(spacing: 6) {
                ...
                nrRow
                freqRow
                if !(vm.useCivMode && vm.civConnected) { txOfsRow }
    ...
    private func formatOfsHz(_ hz: Int64) -> String {
        if hz == 0 { return "0 Hz" }
        let sign = hz > 0 ? "+" : ""
        if abs(hz) >= 1000 { return String(format: "%@%.1fkHz", sign, Double(hz) / 1000.0) }
        return "\(sign)\(hz)Hz"
    }

    private var txOfsRow: some View {
        HStack(spacing: 4) {
            VStack(spacing: 1) {
                Text("TX OFS")
                    .font(.caption2.bold())
            ...
            Button("◀") { vm.setCwTxShift(vm.cwTxShiftHz - Int64(cwTxOfsStep)) }
                .buttonStyle(CwCompactButtonStyle(color: Color(white: 0.2)))
                .simultaneousGesture(LongPressGesture(minimumDuration: 0.4).onEnded { _ in
                    vm.setCwTxShift(vm.cwTxShiftHz - Int64(cwTxOfsStep) * 10)
                })
            Text(formatOfsHz(vm.cwTxShiftHz))
                .font(.system(.body, design: .monospaced).bold())
