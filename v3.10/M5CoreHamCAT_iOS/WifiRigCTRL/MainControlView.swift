...
struct MainControlView: View {
    ...
    @State private var showTxOfsStepPicker = false
    @AppStorage("cwTxOfsStep") private var cwTxOfsStep: Int = 500
    ...
    var body: some View {
        ...
        .onChange(of: vm.sharedMode) { _, mode in
            if !mode.uppercased().contains("CW"), vm.cwTxShiftHz != 0 {
                vm.setCwTxShift(0)
            }
        }
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    ...
                    Button("Admin (Update Pi)") { vm.path.append(.admin) }
                        .disabled(vm.useCivMode && vm.civConnected)
                    Button("Help") { vm.path.append(.help) }
        ...
        .confirmationDialog("Mode", isPresented: $showModePicker) {
            ForEach(vm.supportedModes, id: \.self) { m in
                Button(m) {
                    Task { await vm.setMode(m, width: vm.sharedWidth) }
                    vm.selectedStepIndex = AppConstants.defaultStepIndex(forMode: m)
    ...
    private var portraitLayout: some View {
        VStack(spacing: 6) {
            headerCard
            if isCwMode && !(vm.useCivMode && vm.civConnected) {
                cwTxShiftBar
            }
            meterBar
            panelGrid
        ...
        VStack(spacing: 4) {
            ...
            if vm.sharedTx {
                ...
                HStack(spacing: 4) {
                    ...
                    TxBarMeterView(
                        value: vm.sharedAlc,
                        activeColors: [Color(red: 0.84, green: 0, blue: 0), Color(red: 0.84, green: 0, blue: 0)],
    ...
    // MARK: - CW TX SHIFT bar (CW/CWR mode only)

    private func formatShiftHz(_ hz: Int64) -> String {
        if hz == 0 { return "0" }
        let abs = Swift.abs(hz)
        let sign = hz > 0 ? "+" : "-"
        if abs < 1000 { return "\(sign)\(abs)Hz" }
        return String(format: "%@%.1fkHz", sign, Double(hz) / 1000.0)
    }

    private var cwTxShiftBar: some View {
        HStack(spacing: 6) {
            VStack(spacing: 1) {
                Text("TX OFS")
                    .font(.system(size: 10, weight: .semibold))
                    .foregroundStyle(Color(red: 0.27, green: 0.4, blue: 0.27))
                Text(cwTxOfsStep < 1000 ? "\(cwTxOfsStep)Hz" : "\(cwTxOfsStep / 1000)kHz")
                    .font(.system(size: 9))
                    .foregroundStyle(Color(red: 0.27, green: 0.4, blue: 0.27))
            }
            .onTapGesture { vm.setCwTxShift(0) }
            .onLongPressGesture { showTxOfsStepPicker = true }
            Button("◀") { vm.setCwTxShift(vm.cwTxShiftHz - Int64(cwTxOfsStep)) }
                .frame(maxWidth: .infinity)
                .buttonStyle(.bordered).tint(Color(red: 0.1, green: 0.23, blue: 0.1))
                .simultaneousGesture(LongPressGesture().onEnded { _ in vm.setCwTxShift(vm.cwTxShiftHz - Int64(cwTxOfsStep) * 10) })
            Text(formatShiftHz(vm.cwTxShiftHz))
                .frame(maxWidth: .infinity)
                .font(.system(size: 11, weight: .semibold, design: .monospaced))
                .foregroundStyle(vm.cwTxShiftHz != 0 ? Color(red: 1.0, green: 0.84, blue: 0.0) : Color(red: 0.53, green: 0.8, blue: 0.53))
            Button("▶") { vm.setCwTxShift(vm.cwTxShiftHz + Int64(cwTxOfsStep)) }
                .frame(maxWidth: .infinity)
                .buttonStyle(.bordered).tint(Color(red: 0.1, green: 0.23, blue: 0.1))
                .simultaneousGesture(LongPressGesture().onEnded { _ in vm.setCwTxShift(vm.cwTxShiftHz + Int64(cwTxOfsStep) * 10) })
        ...
        .padding(.vertical, 4)
        .background(Color(red: 0.027, green: 0.059, blue: 0.027))
        .cornerRadius(6)
        .confirmationDialog("TX OFS Step", isPresented: $showTxOfsStepPicker) {
            ForEach([10, 50, 100, 500, 1000], id: \.self) { step in
                Button(step < 1000 ? "\(step)Hz" : "\(step / 1000)kHz") { cwTxOfsStep = step }
            }
            Button("Cancel", role: .cancel) {}
        }
    }
...
// MARK: - TX Meter view (PO / ALC)

struct TxBarMeterView: View {
    let value: Double   // 0.0…1.0
    let activeColors: [Color]  // gradient from low to high bar
    ...
    var body: some View {
        GeometryReader { _ in
            HStack(spacing: 2) {
                ForEach(0..<totalBars, id: \.self) { i in
                    let threshold = Int(value.clamped(to: 0...1) * Double(totalBars))
                    let active = i < threshold
                    let c = active ? activeColor(for: i) : inactiveColor
                    RoundedRectangle(cornerRadius: 2)
                        .fill(LinearGradient(colors: [c, c.opacity(active ? 0.75 : 0.6)],
                                             startPoint: .top, endPoint: .bottom))
                        .shadow(color: active ? c.opacity(0.8) : .clear, radius: active ? 2 : 0)
    ...
    private func activeColor(for i: Int) -> Color {
        guard activeColors.count >= 2 else { return activeColors.first ?? .orange }
        let t = Double(i) / Double(totalBars - 1)
        return activeColors.count == 3
            ? (t < 0.6 ? activeColors[0] : (t < 0.85 ? activeColors[1] : activeColors[2]))
            : activeColors[0]
    }
}
                ...
                Section(group.band) {
                    ForEach(group.entries) { item in
                        MemoryRowView(entry: item.entry, showLockIcon: false)
                            .contentShape(Rectangle())
                            .onTapGesture {
                                let isFt = ["FT8","FT4"].contains(item.entry.mode.uppercased())
                                if isFt { vm.ft8IsFt4 = item.entry.mode.uppercased() == "FT4" }
                                Task { await vm.recallMemory(item.entry) }
                                dismiss()
                                if isFt {
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
                                        if vm.path.last != .ft8 { vm.path.append(.ft8) }
                                    }
                                } else {
                                    if vm.path.last == .ft8 { vm.path.removeLast() }
                                }
                            }
        ...
        List {
            ForEach(Array(vm.userMemories.enumerated()), id: \.offset) { idx, entry in
                MemoryRowView(entry: entry)
                    .contentShape(Rectangle())
                    .onTapGesture {
                        let isFt = ["FT8","FT4"].contains(entry.mode.uppercased())
                        if isFt { vm.ft8IsFt4 = entry.mode.uppercased() == "FT4" }
                        Task { await vm.recallMemory(entry) }
                        dismiss()
                        if isFt {
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
                                if vm.path.last != .ft8 { vm.path.append(.ft8) }
...
/// Format frequency with enough decimal places to show sub-kHz components unambiguously.
/// e.g. 7005000 Hz → "7.005 MHz", 7005500 Hz → "7.0055 MHz"
private func formatSpotFreqMHz(_ hz: Int64) -> String {
    let mhz = Double(hz) / 1_000_000.0
    if hz % 1_000 == 0 { return String(format: "%.3f MHz", mhz) }
    if hz % 100  == 0  { return String(format: "%.4f MHz", mhz) }
    return String(format: "%.5f MHz", mhz)
}
        ...
        VStack(spacing: 0) {
            ...
            if isLoading {
                ...
                List(filteredSpots) { spot in
                    PotaSpotRow(spot: spot, hunterStore: vm.hunterStore).contentShape(Rectangle())
                        .onTapGesture {
                            let raw = spot.mode.uppercased()
                            let isFt = raw == "FT8" || raw == "FT4"
                            if isFt { vm.ft8IsFt4 = raw == "FT4" }
...
struct PotaSpotRow: View {
    let spot: PotaSpot
    var hunterStore: HunterStore? = nil
    private var isHunted: Bool { hunterStore?.isHunted(spot.reference) ?? false }
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 8) {
                Text(spot.activator).bold()
                Text(formatSpotFreqMHz(spot.freqHz)).font(.callout.monospacedDigit())
                ...
                if isHunted {
                    Text("P✓")
                        ...
                        .background(RoundedRectangle(cornerRadius: 3).fill(Color(hex: 0x1B5E20)))
                }
            }
            .font(.callout)
...
// MARK: - SOTA Cluster (REST API v2)

enum SotaCluster {
    static func fetchSpots() async -> [SotaSpot] {
        guard let url = URL(string: "https://api2.sota.org.uk/api/spots/-2/all/all") else { return [] }
...
struct MemorySotaView: View {
    ...
    private var availableAreas: [String] {
        Array(Set(spots.compactMap { s -> String? in
            let a = s.effectiveAssocCode; return a.isEmpty ? nil : a
        })).sorted()
    }
    ...
    private func fetch() async {
        isLoading = true; errorMsg = nil
        let rawSpots = await SotaCluster.fetchSpots()
        let deduped = Dictionary(grouping: rawSpots, by: { "\($0.activatorCallsign ?? "")|\($0.summitCode ?? "")" })
            .values.compactMap { $0.max(by: { $0.id < $1.id }) }
        spots = deduped.sorted { $0.freqMhz < $1.freqMhz }
        vm.updateSotaCache(spots)
        if !modeFilter.isEmpty && !availableModes.contains(modeFilter) { modeFilter = "" }
        if !bandFilter.isEmpty && !availableBands.contains(bandFilter) { bandFilter = "" }
