...
final class MainViewModel {
    ...
    var piVersionMismatch: Bool = false


    // Connection state
    var isConnectedToRig: Bool = false {
        didSet { if isConnectedToRig { applyPendingIfConnected() } }
    }
    var isBusy: Bool = false

    // SP2ALART連携: 接続前に受け取った周波数・モードをpendingとして保持
    private var pendingFreqHz: Int64 = 0
    private var pendingSpotMode: String? = nil

    func setPendingFreqMode(freqHz: Int64, spotMode: String) {
        pendingFreqHz = freqHz
        pendingSpotMode = spotMode
        applyPendingIfConnected()
    }

    private func applyPendingIfConnected() {
        guard isConnectedToRig,
              pendingFreqHz > 0,
              let spotMode = pendingSpotMode else { return }
        let freqHz = pendingFreqHz
        let upper = spotMode.uppercased().trimmingCharacters(in: .whitespaces)
        let isFt = upper == "FT8" || upper == "FT4"
        let (rawMode, width) = Self.mapSpotMode(spotMode, freqHz: freqHz)
        let rigMode = resolveMode(rawMode, freqHz: freqHz)
        pendingFreqHz = 0
        pendingSpotMode = nil
        if isFt { ft8IsFt4 = (upper == "FT4") }
        Task { @MainActor [weak self] in
            await self?.setMode(rigMode, width: width)
            await self?.setFreq(freqHz)
            guard let self else { return }
            if isFt && self.path.last != .ft8 { self.path.append(.ft8) }
        }
    }

    /** SSBモードをリグが持たない場合、周波数に応じて USB/LSB に変換する */
    func resolveMode(_ mode: String, freqHz: Int64) -> String {
        if mode.uppercased() == "SSB" &&
           !supportedModes.contains(where: { $0.uppercased() == "SSB" }) {
            return freqHz >= 10_000_000 ? "USB" : "LSB"
        }
        return mode
    }

    // POTAスポットモード → 無線機モード変換
    static func mapSpotMode(_ spotMode: String, freqHz: Int64) -> (String, Int) {
        func usbOrLsb() -> String { freqHz >= 10_000_000 ? "USB" : "LSB" }
        switch spotMode.uppercased().trimmingCharacters(in: .whitespaces) {
        case "CW", "CW-R":                                          return ("CW",        500)
        case "FM", "C4FM", "D-STAR", "DSTAR", "DMR":               return ("FM",      15000)
        case "AM":                                                   return ("AM",       6000)
        case "FT8", "FT4", "JS8", "RTTY", "PSK31",
             "PSKR", "DIGI", "MSK144":                              return (usbOrLsb(), 3000)
        case "SSB":                                                  return ("SSB",      3000)
        case "USB":                                                  return ("USB",      3000)
        case "LSB":                                                  return ("LSB",      3000)
        default:                                                     return (usbOrLsb(), 3000)
        }
    }
    var errorMessage: String?

    // MARK: - POTA/SOTA spot cache + hunter store (v3.01)

    var hunterStore = HunterStore()

    // Activator map keyed by uppercased BASE callsign (suffix after "/" stripped).
    // Populated when MemoryPotaView / MemorySotaView fetches live spots.
    private(set) var potaActivatorMap: [String: [PotaSpot]] = [:]
    private(set) var sotaActivatorSet: Set<String> = []

    func updatePotaCache(_ spots: [PotaSpot]) {
        ...
        potaActivatorMap = map
    }

    func updateSotaCache(_ spots: [SotaSpot]) {
        sotaActivatorSet = Set(spots.compactMap {
            $0.activatorCallsign?.uppercased().components(separatedBy: "/").first
        })
    }
    ...
    var sharedRfOut: Double = 0
    var sharedAlc: Double = 0
    ...
    // -1=always on, 0=system default, 5/15/30/60=minutes until screen off (resets on poll activity)
    var screenTimeoutMinutes: Int = {
        let ud = UserDefaults.standard
        if let stored = ud.object(forKey: "screenTimeoutMinutes") as? Int { return stored }
        if let old = ud.object(forKey: "keepScreenAwake") as? Bool { return old ? -1 : 0 }
        return 15
    }()
    private var _lastActivityTime: Date = Date()
    private var _screenMonitorTask: Task<Void, Never>?

    // VFO state (v2.20)
    var vfoMode: String = ""      // "ab" | "mainsub"
    var vfoCurrentSide: String = ""  // "A"/"B" or "Main"/"M/S"/"S/M"

    // Repeater settings (persisted)
    var repeaterOffsetDir: String = UserDefaults.standard.string(forKey: "repeaterOffsetDir") ?? ""
    var repeaterOffsetHz: Int64 = Int64(UserDefaults.standard.integer(forKey: "repeaterOffsetHz"))
    var repeaterToneMode: String = UserDefaults.standard.string(forKey: "repeaterToneMode") ?? ""
    var repeaterToneHz: Double = (UserDefaults.standard.object(forKey: "repeaterToneHz") as? Double) ?? 0.0

    /// TX output frequency: RX freq ± repeater offset during transmit, else RX freq.
    var displayFreq: Int64 {
        guard txEnabled, !repeaterOffsetDir.isEmpty, repeaterOffsetHz > 0 else { return sharedFreq }
        return repeaterOffsetDir == "-" ? sharedFreq - repeaterOffsetHz : sharedFreq + repeaterOffsetHz
    }

    func setRepeater(toneMode: String, toneHz: Double, offsetDir: String, offsetHz: Int64) {
        repeaterOffsetDir = offsetDir; UserDefaults.standard.set(offsetDir, forKey: "repeaterOffsetDir")
        repeaterOffsetHz  = offsetHz;  UserDefaults.standard.set(Int(offsetHz), forKey: "repeaterOffsetHz")
        repeaterToneMode  = toneMode;  UserDefaults.standard.set(toneMode, forKey: "repeaterToneMode")
        repeaterToneHz    = toneHz;    UserDefaults.standard.set(toneHz, forKey: "repeaterToneHz")
    }

    func clearRepeater() { setRepeater(toneMode: "", toneHz: 0, offsetDir: "", offsetHz: 0) }

    var cwTxShiftHz: Int64 = 0

    func setCwTxShift(_ shiftHz: Int64) {
        cwTxShiftHz = shiftHz
        if isDemoMode { return }
        if useCivMode {
            // CI-V mode: DUP+/DUP- interferes with CW TX — force Simplex
            if civ.isConnected { Task { civ.setTxShift(0) } }
            return
        }
        Task {
            do { try await api.setTxShift(shiftHz: shiftHz) } catch { }
        }
    }
    ...
    // Default filter widths (v2.31)
    var defaultSsbWidth: Int {
        get { UserDefaults.standard.integer(forKey: "defaultSsbWidth").nonZeroOr(3000) }
        set { UserDefaults.standard.set(newValue, forKey: "defaultSsbWidth") }
    }
    var defaultCwWidth: Int {
        get { UserDefaults.standard.integer(forKey: "defaultCwWidth").nonZeroOr(500) }
        set { UserDefaults.standard.set(newValue, forKey: "defaultCwWidth") }
    }

    func modeDefaultWidth(_ mode: String) -> Int {
        let m = mode.uppercased()
        if m == "FM" || m == "PKTFM" { return 12000 }
        if m == "FMN"                { return 9000 }
        if m.hasPrefix("CW")         { return defaultCwWidth }
        if m == "USB" || m == "LSB"  { return defaultSsbWidth }
        return 0
    }
    ...
    var ft8AudioFreqHz: Int = {
        let v = UserDefaults.standard.integer(forKey: "ft8AudioFreqHz")
        return v > 0 ? v : 1500
    }()
    var ft8DecodeDepth: Int = {
        let v = UserDefaults.standard.integer(forKey: "ft8DecodeDepth")
        return v > 0 ? v : 1
    }()
    var ft8AutoTxMode: String = UserDefaults.standard.string(forKey: "ft8AutoTxMode") ?? "even"
    var ft8Filter: Bool = UserDefaults.standard.bool(forKey: "ft8Filter")
    ...
    private func handleBleKeyForFmMode(isOn: Bool) {
        let bleMode = cwBle.currentMode.uppercased()
        guard bleMode.contains("FM") || bleMode == "DSTAR" || bleMode == "D-STAR" else {
        ...
        let isCiv = useCivMode && civ.isConnected
        if isOn {
            fmTailTask?.cancel(); fmTailTask = nil
            if isCiv {
                // CI-V: PTT + 700Hz トーンを RS-BA1 TX audio 経由で IC-705 へ送信
                if !fmTxActive {
                    fmTxActive = true
                    txEnabled = true
                    civ.prepareFmCwTone()  // トーンスレッド準備（非ブロッキング）
                }
                civ.civPttDown()
                civ.fmCwKeyOn()
            } else {
                // WiFi: 700Hz tone to Pi API + PTT via Hamlib
            ...
            if isCiv {
                civ.fmCwKeyOff()  // トーン即停止
            } else {
                cwAudioStream.keyOff()
            }
            fmTailTask?.cancel()
            fmTailTask = Task { [weak self, isCiv] in
                try? await Task.sleep(nanoseconds: 500_000_000)
                guard let self, !Task.isCancelled else { return }
                if isCiv {
                    self.civ.civPttUp()
                    self.civ.endFmCwTone()
                } else {
    ...
    func persistConnectionSettings() {
        ...
        d.set(screenTimeoutMinutes, forKey: "screenTimeoutMinutes")
    ...
    func openSelectedRig() async -> String? {
        ...
        do {
            ...
            // Restart cw_bridge.py on Pi so CW keying works immediately.
            // Needed when Pi was restarted while BLE keyer remained connected (no BLE reconnect event).
            Task { _ = await cwOpen() }
            return nil
        } catch {
            return error.localizedDescription
    ...
    // MARK: - Screen timeout management

    func applyWakeLock() {
        _screenMonitorTask?.cancel()
        _screenMonitorTask = nil
        switch screenTimeoutMinutes {
        case -1:
        ...
        default:
            ...
            let minutes = screenTimeoutMinutes
            _screenMonitorTask = Task { [weak self] in
                while !Task.isCancelled {
                    try? await Task.sleep(nanoseconds: 30_000_000_000)
                    guard let self, !Task.isCancelled else { return }
                    let elapsed = Date().timeIntervalSince(self._lastActivityTime)
                    UIApplication.shared.isIdleTimerDisabled = elapsed < Double(minutes) * 60
                }
            }
        }
    }
    ...
    func noteActivity() {
        _lastActivityTime = Date()
        if screenTimeoutMinutes > 0, !UIApplication.shared.isIdleTimerDisabled {
            UIApplication.shared.isIdleTimerDisabled = true
        }
    }
    ...
    func civConnect() async -> String? {
        ...
        // CI-V stream is now confirmed open — reset DUP to Simplex
        Task { civ.setTxShift(0) }
        return nil
    }
    ...
    var pollSuppressed: Bool = false
    private static let expectedPiApiVersion = "3.10"
    ...
    private func pollOnce() async {
        ...
        if useCivMode, civ.isConnected {
            ...
            if let ro = s.rfOut { self.sharedRfOut = Double(ro) } else if !s.tx { self.sharedRfOut = 0 }
            if let al = s.alc  { self.sharedAlc   = Double(al) } else if !s.tx { self.sharedAlc   = 0 }
            loggedFirstPollError = false
            consecutivePollFailures = 0
            noteActivity()
            return
        ...
        do {
            ...
            if let v = s.api_version, !v.isEmpty { self.piVersionMismatch = (v != Self.expectedPiApiVersion) }
            if let v = s.rf_out, s.tx { self.sharedRfOut = v } else if !s.tx { self.sharedRfOut = 0 }
            if let v = s.alc,    s.tx { self.sharedAlc   = v } else if !s.tx { self.sharedAlc   = 0 }
            loggedFirstPollError = false
            consecutivePollFailures = 0
            noteActivity()
        } catch {
    ...
    func setMode(_ mode: String, width: Int) async {
        ...
        if isDemoMode { return }
        // "USB-D" is the display label for PKTUSB (IC-7300/IC-705 data mode).
        let modeToSend = (mode.uppercased() == "USB-D") ? "PKTUSB" : mode
        if useCivMode, civ.isConnected {
            _ = await Task.detached(priority: .userInitiated) { [civ = self.civ, modeToSend, effectiveWidth] in civ.setMode(modeToSend, width: effectiveWidth) }.value
            return
        }
        do { try await api.setMode(mode: modeToSend, width: effectiveWidth) }
    ...
    func persistFt8Settings() {
        ...
        d.set(ft8AudioFreqHz, forKey: "ft8AudioFreqHz")
        d.set(ft8DecodeDepth, forKey: "ft8DecodeDepth")
        d.set(ft8AutoTxMode, forKey: "ft8AutoTxMode")
        d.set(ft8Filter, forKey: "ft8Filter")
        d.set(ft8IsFt4, forKey: "ft8IsFt4")
    }
    ...
    func enterFt8Mode() async {
        if useCivMode, civ.isConnected {
            let mode = ft8TxMode; let freq = ft8LastFreq
            _ = await Task.detached(priority: .userInitiated) { [civ = self.civ] in
                _ = civ.setMode(mode, width: 3000)
                if freq > 0 { _ = civ.setFrequency(freq) }
            }.value
            return
        }
        do {
    ...
    func exitFt8Mode() async {
        ...
        persistFt8Settings()
        if useCivMode, civ.isConnected {
            let mode = preFt8Mode; let freq = preFt8Freq; let width = preFt8Width
            _ = await Task.detached(priority: .userInitiated) { [civ = self.civ] in
                if !mode.isEmpty { _ = civ.setMode(mode, width: width) }
                if freq > 0 { _ = civ.setFrequency(freq) }
            }.value
        } else {
            do {
    ...
    func setFt8TxMode(_ m: String) async {
        ...
        persistFt8Settings()
        if useCivMode, civ.isConnected {
            _ = await Task.detached(priority: .userInitiated) { [civ = self.civ, m] in
                _ = civ.setMode(m, width: 3000)
            }.value
            return
        }
    ...
    // MARK: FT8 native API wrappers (v3.00)

    func ft8Start() async throws { try await api.ft8Start() }
    func ft8Stop()  async throws { try await api.ft8Stop() }

    func ft8SetAutoTx(active: Bool, mode: String, msg: String, audioFreqHz: Int) async throws {
        try await api.ft8SetAutoTx(active: active, mode: mode, msg: msg, audioFreqHz: audioFreqHz)
    }

    func ft8CqAuto(active: Bool, msg: String, mode: String, audioFreqHz: Int,
                   myCall: String, myGrid: String) async throws {
        try await api.ft8CqAuto(active: active, msg: msg, mode: mode, audioFreqHz: audioFreqHz,
                                myCall: myCall, myGrid: myGrid)
    }

    func ft8StartQso(firstMsg: String, dxCall: String, myCall: String, txMode: String,
                     audioFreqHz: Int, initialState: Int = 0, stepSnrMsg: String = "",
                     step2Msg: String = "", step3Msg: String = "", step4Msg: String = "") async throws {
        try await api.ft8StartQso(firstMsg: firstMsg, dxCall: dxCall, myCall: myCall,
                                   txMode: txMode, audioFreqHz: audioFreqHz,
                                   initialState: initialState, stepSnrMsg: stepSnrMsg,
                                   step2Msg: step2Msg, step3Msg: step3Msg, step4Msg: step4Msg)
    }

    func ft8CancelQso() async throws { try await api.ft8CancelQso() }

    func ft8SetDepth(_ depth: Int) async throws { try await api.ft8SetDepth(depth) }

    func ft8SetFt4(_ isFt4: Bool) async throws { try await api.ft8SetFt4(isFt4) }

    func ft8SetTxGain(_ gain: Float) async throws { try await api.ft8SetTxGain(gain) }

    func ft8SetDecodeFilter(freqs: [Int], bw: Int = 150) async throws {
        try await api.ft8SetDecodeFilter(freqs: freqs, bw: bw)
    }

    func ft8DecodeDebug() async throws -> String { try await api.ft8DecodeDebug() }

    func makeFt8SseRequest(path: String) async -> URLRequest? {
        try? await api.makeSseRequest(path: path)
    }
    ...
    func snapshotCurrentProfile(name: String) -> ProfileConfig {
        ProfileConfig(
            ...
            useCivMode: useCivMode,
            civHost: civHost,
            civUsername: civUsername,
            civPassword: civPassword,
            civPort1: civPort1,
            civPort2: civPort2,
            civPort3: civPort3,
            civAddressHex: civAddressHex
        )
    }
    ...
    func loadProfile(_ p: ProfileConfig) {
        ...
        useCivMode = p.useCivMode
        civHost = p.civHost
        civUsername = p.civUsername
        civPassword = p.civPassword
        civPort1 = p.civPort1
        civPort2 = p.civPort2
        civPort3 = p.civPort3
        civAddressHex = p.civAddressHex
        ProfileStore.activeId = p.id
    ...
    func cwSendMorse() async -> String? {
        ...
        // FM mode: pre-build full CW PCM and stream to Pi audio endpoint (same as CI-V path)
        if sharedMode.uppercased().contains("FM") || sharedMode.uppercased() == "DSTAR" || sharedMode.uppercased() == "D-STAR" {
            cwMorseSending = true
            startLocalCwSidetone(text: text, wpm: cwWpm)
            let ditMs = 1200.0 / Double(max(5, min(60, cwWpm)))
            let pcm = buildCwPcm(text: text.uppercased(), ditMs: ditMs)
            try? await api.setPtt(state: true)
            await streamCwPcmToWifi(pcm: pcm)
        ...
        cwTextTask = Task { @MainActor [weak self] in
            ...
            // Skip cleanup if cancelled — the canceller (new send or cwStopMorse) owns cleanup.
            guard !Task.isCancelled else { return }
            self.cwMorseSending = false
    ...
    private func civCwSendMorse(text: String) async -> String? {
        ...
        let ditMs = 1200.0 / Double(max(5, min(60, cwWpm)))
        // IC-705 internal keyer (cmd 0x17) only works in CW mode.
        // FM mode must use PCM audio streaming regardless of BK-IN setting.
        let isFm = sharedMode.uppercased().contains("FM")
            || sharedMode.uppercased() == "DSTAR" || sharedMode.uppercased() == "D-STAR"
        let isBkIn = isFm ? false : await Task.detached(priority: .userInitiated) { [civ = self.civ] in
            civ.getBkIn() ?? false
        }.value
        ...
        if isBkIn {
            ...
            // FM-CW: 完全なCW PCM（700Hzトーン＋無音）を事前生成し RS-BA1 TX audio に直接ストリーミング
            // Android の sendCwTextCiv (non-CW mode path) と同じ方式
            let pcm = buildCwPcm(text: text.uppercased(), ditMs: ditMs)
            _ = await Task.detached(priority: .userInitiated) { [civ = self.civ] in
                civ.setPtt(true)
            }.value
            civ.stopTxAudioCapture()  // マイク入力を止め、CWトーンのみを送る
            let chunkSize = 320       // 20ms @ 8000 Hz (160 samples × 2 bytes)
            var offset = 0
            var nextMs = Int64(Date().timeIntervalSince1970 * 1000)
            while offset < pcm.count {
                if Task.isCancelled { break }
                let end = min(offset + chunkSize, pcm.count)
                civ.sendTxAudioDirect(pcm.subdata(in: offset..<end))
                offset = end
                nextMs += 20
                let sleepMs = nextMs - Int64(Date().timeIntervalSince1970 * 1000)
                if sleepMs > 1 { try? await Task.sleep(nanoseconds: UInt64(sleepMs) * 1_000_000) }
            }
            try? await Task.sleep(nanoseconds: 600_000_000)
            _ = await Task.detached(priority: .userInitiated) { [civ = self.civ] in
                _ = civ.setPtt(false)
            ...
            // CW mode PTT-per-element (BK-IN OFF)
    ...
    /// Android の buildCwPcm() 相当: 700Hz サイン波（要素ON）と無音（要素OFF）を連結した 8kHz 16bit PCM を生成する。
    private func buildCwPcm(text: String, ditMs: Double, sampleRate: Int = 8000) -> Data {
        let seq = cwMorseSequence(for: text, unitMs: ditMs)
        var pcm = Data()
        var phase = 0.0
        for (isOn, durMs) in seq {
            let nSamples = max(1, Int(durMs * Double(sampleRate) / 1000.0))
            var chunk = Data(count: nSamples * 2)
            chunk.withUnsafeMutableBytes { raw in
                let dst = raw.baseAddress!.assumingMemoryBound(to: Int16.self)
                for i in 0..<nSamples {
                    if isOn {
                        phase += 2.0 * .pi * 700.0 / Double(sampleRate)
                        dst[i] = Int16(clamping: Int(32767.0 * 0.7 * sin(phase)))
                    } else {
                        dst[i] = 0
                    }
                }
            }
            if isOn { phase = phase.truncatingRemainder(dividingBy: 2.0 * .pi) }
            pcm.append(chunk)
        }
        return pcm
    }

    /// WiFi FM-CW: pre-built PCM を Pi の /radio/audio_tx へストリーミング送信する。
    /// CwAudioStream と同じ bound-streams + DispatchQueue パターン (Swift Task ではブロッキング書き込みがコープスレッドを占有するため)。
    private func streamCwPcmToWifi(pcm: Data) async {
        let host = hostName
        let port = apiPort
        let key = apiKey
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            guard !host.isEmpty,
                  let url = URL(string: "http://\(host):\(port)/radio/audio_tx?rate=8000") else {
                continuation.resume()
                return
            }
            var inS: InputStream?
            ...
            task.resume()
            DispatchQueue(label: "cw.wifi.pcm", qos: .userInitiated).async {
                let chunkSize = 160  // 10ms @ 8000 Hz
                var offset = 0
                var nextMs = Date().timeIntervalSince1970 * 1000
                while offset < pcm.count {
                    nextMs += 10
                    let end = min(offset + chunkSize, pcm.count)
                    ...
                    var waited = 0
                    while waited < 100 {
                        if outputStream.hasSpaceAvailable { break }
                        Thread.sleep(forTimeInterval: 0.005)
                        waited += 1
                    }
                    _ = chunk.withUnsafeBytes { ptr -> Int in
                        guard let base = ptr.baseAddress?.assumingMemoryBound(to: UInt8.self) else { return 0 }
                        return outputStream.write(base, maxLength: chunk.count)
                    }
                    offset = end
                    let sleepMs = nextMs - Date().timeIntervalSince1970 * 1000
                    if sleepMs > 0 { Thread.sleep(forTimeInterval: sleepMs / 1000.0) }
                }
                outputStream.close()
                task.cancel()
    ...
    var userMemories: [MemoryEntry] {
        guard let data = UserDefaults.standard.data(forKey: "user_memories_json"),
              let list = try? JSONDecoder().decode([MemoryEntry].self, from: data)
        else { return [] }
        return list
    }

    func saveUserMemories(_ list: [MemoryEntry]) {
        if let data = try? JSONEncoder().encode(list) {
            UserDefaults.standard.set(data, forKey: "user_memories_json")
        }
    }

    var customPresets: [MemoryEntry] {
        guard let data = UserDefaults.standard.data(forKey: "custom_presets_json"),
              let list = try? JSONDecoder().decode([MemoryEntry].self, from: data),
              !list.isEmpty
        else { return PRESET_MEMORIES }
        return list
    }

    func saveCustomPresets(_ list: [MemoryEntry]) {
        if let data = try? JSONEncoder().encode(list) {
            UserDefaults.standard.set(data, forKey: "custom_presets_json")
        }
    }

    func resetCustomPresets() {
        UserDefaults.standard.removeObject(forKey: "custom_presets_json")
    }

    func recallMemory(_ entry: MemoryEntry) async {
        try? await Task.sleep(nanoseconds: 100_000_000)  // 0.1s — screen transition
        let resolvedMode = entry.mode.isEmpty ? "" : resolveMode(entry.mode, freqHz: entry.freqHz)
        let w = resolvedMode.isEmpty ? 0 : modeDefaultWidth(resolvedMode)
        if !resolvedMode.isEmpty {
            for _ in 0..<2 {
                try? await Task.sleep(nanoseconds: 150_000_000)  // 150ms
                await setMode(resolvedMode, width: w > 0 ? w : sharedWidth)
            }
            let stepIdx = entry.stepIndex >= 0 ? entry.stepIndex
                        : AppConstants.defaultStepIndex(forMode: resolvedMode)
            selectedStepIndex = stepIdx
            rememberStepForCurrentMode()
        }
        for _ in 0..<3 {
            try? await Task.sleep(nanoseconds: 150_000_000)  // 150ms
            if !resolvedMode.isEmpty {
                await setMode(resolvedMode, width: w > 0 ? w : sharedWidth)
                await setFreq(entry.freqHz)
            } else {
                await setFreq(entry.freqHz)
            }
        }
    ...
    func adminUpdatePi(apiData: Data, scriptData: Data) async -> String? {
        ...
        for _ in 1...5 {
            try? await Task.sleep(nanoseconds: 5_000_000_000)
            do { try await api.sendApiUpdate(apiData); apiResent = true; break }
            catch { }
        }
        guard apiResent else { return "setup OK / api.py resend failed" }
        adminSetupLog += "\n⑤ 新しい api.py の起動待ち…"
        for attempt in 1...6 {
            try? await Task.sleep(nanoseconds: 5_000_000_000)
            adminSetupLog = "\n⑤ 起動待ち (\(attempt * 5)s)…"
            let v = await api.getServerVersion()
            if v.apiVersion != "?" { break }
        }
        return nil
    }

    // MARK: - Admin (mfsk build log — v3.01)

    var adminMfskLog: String = ""
    var adminMfskRunning: Bool = false

    func adminPollMfskLog() async {
        if isDemoMode { adminMfskLog = "[DEMO] mfsk-decode build ok\n=== 完了 ===\n"; return }
        await adminEnsureConfigured()
        if let (running, log) = try? await api.getMfskBuildLog() {
            adminMfskRunning = running
            adminMfskLog = log
        }
    }

    /// Pi リブート待機（オフライン検出 → 再起動完了まで最大150秒）
    func rebootPiAndWait() async -> String {
        if isDemoMode { return "Pi reboot complete (demo)" }
        await adminEnsureConfigured()
        var rebooted = false
        let readyDeadline = Date().addingTimeInterval(30)
        while Date() < readyDeadline {
            if await api.rebootPi() { rebooted = true; break }
            try? await Task.sleep(nanoseconds: 3_000_000_000)
        }
        guard rebooted else { return "Reboot failed — Pi unreachable" }
        let offlineDeadline = Date().addingTimeInterval(45)
        while Date() < offlineDeadline {
            try? await Task.sleep(nanoseconds: 3_000_000_000)
            if (try? await api.getStatus()) == nil { break }
        }
        let onlineDeadline = Date().addingTimeInterval(150)
        while Date() < onlineDeadline {
            if (try? await api.getStatus()) != nil { return "Pi reboot complete" }
            try? await Task.sleep(nanoseconds: 5_000_000_000)
        }
        return "Pi reboot timeout — check connection"
    }

    /// Update Pi 後に mfsk-decode をバックグラウンドビルドさせる
    func triggerMfskBuildAfterUpdate() async -> String {
        if isDemoMode { adminMfskLog = "[DEMO] mfsk build started"; return "mfsk build started (demo)" }
        await adminEnsureConfigured()
        for _ in 1...20 {
            try? await Task.sleep(nanoseconds: 3_000_000_000)
            if (try? await api.triggerMfskBuild(force: true)) != nil {
                let deadline = Date().addingTimeInterval(15)
                while Date() < deadline {
                    try? await Task.sleep(nanoseconds: 2_000_000_000)
                    if let (running, log) = try? await api.getMfskBuildLog() {
                        adminMfskLog = log
                        if running || log.contains("ビルド開始") { return "mfsk build started" }
                    }
                }
                return "mfsk build started (log pending)"
            }
        }
        return "mfsk build trigger failed (Pi unreachable?)"
    }

    /// Pi の時刻を Android / NTP と同期する
    func syncPiTime() async -> String {
        if isDemoMode { return "Sync OK[demo]" }
        await adminEnsureConfigured()
        let nowMs = Int64(Date().timeIntervalSince1970 * 1000)
        guard let (ok, source, drift) = try? await api.syncTime(ms: nowMs), ok else {
            return "Sync failed"
        }
        let s = drift > 0 ? "+\(drift)" : "\(drift)"
        return "Sync OK[\(source)] (drift \(s)ms)"
    }
