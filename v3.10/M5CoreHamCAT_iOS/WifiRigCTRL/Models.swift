...
nonisolated struct RigStatus: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.rf_out = Self.decodeDouble(c, .rf_out)
        self.alc = Self.decodeDouble(c, .alc)
    }
...
nonisolated enum AppConstants {
    ...
    static func defaultStepIndex(forMode mode: String) -> Int {
        ...
        if m.contains("FM") || m == "DSTAR" || m == "D-STAR" || m == "C4FM" { return 7 } // 20 kHz
        if m.contains("CW") { return 2 } // 100 Hz
        if m.contains("AM") { return 4 } // 1 kHz
        return 2 // 100 Hz (SSB/PKT/RTTY等)
    }
...
nonisolated struct PotaSpot: Identifiable, Decodable {
    ...
    var freqHz:  Int64  { Int64(((Double(frequency) ?? 0.0) * 1000.0).rounded()) }
...
nonisolated struct SotaSpot: Identifiable, Decodable {
    ...
    var freqHz:  Int64  { Int64(((Double(frequency ?? "") ?? 0.0) * 1_000_000).rounded()) }
    var codeStr: String {
        if let a = associationCode, let s = summitCode { return "\(a)/\(s)" }
        if let s = summitCode, s.contains("/") { return s }
        return ""
    }
    var effectiveAssocCode: String {
        if let a = associationCode { return a }
        return summitCode?.components(separatedBy: "/").first ?? ""
    }
    var displayName: String? { summitDetails ?? summitName }

    enum CodingKeys: String, CodingKey {
        case id, timeStamp, comments, activatorCallsign, frequency, mode
        case summitCode, associationCode, summitDetails, summitName
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        summitName        = try? c.decode(String.self, forKey: .summitName)
        if let d = try? c.decodeIfPresent(Double.self, forKey: .frequency) {
            frequency = String(d)
        } else {
            frequency = try? c.decode(String.self, forKey: .frequency)
        }
    ...
    init(id: Int64, timeStamp: String?, comments: String?, activatorCallsign: String?,
         frequency: String?, mode: String?, summitCode: String?, associationCode: String?,
         summitDetails: String?, summitName: String? = nil) {
        self.id                = id
