...
struct RigSelectView: View {
    ...
    var body: some View {
        Form {
            ...
            if vm.piVersionMismatch {
                Section {
                    Label("Pi API version mismatch — update via UPDATE", systemImage: "exclamationmark.triangle.fill")
                        .foregroundStyle(.red)
                        .font(.callout)
                }
            }
        ...
        .onAppear {
            ...
            // Skip if polling was suppressed by auto-disconnect (Pi unreachable) — will resume
            // when the user explicitly opens a rig (openSelectedRig calls startStatusPolling).
            if !vm.demoMode && !vm.useCivMode && !vm.pollSuppressed {
