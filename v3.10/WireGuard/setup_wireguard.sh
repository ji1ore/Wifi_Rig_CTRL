...
if command -v ufw > /dev/null 2>&1 && sudo ufw status | grep -q "Status: active"; then
    echo "=== ufw が有効 → WireGuard サブネットからのアクセスを許可 ==="
    sudo ufw allow from 10.0.0.0/24 to any port 8000 comment "WireGuard FastAPI"
    sudo ufw allow from 10.0.0.0/24 to any port 8443 comment "WireGuard webft8"
    sudo ufw allow from 10.0.0.0/24 to any port 8001 comment "WireGuard audio"
    sudo ufw reload
    echo "ufw ルール追加完了"
else
    echo "=== ufw 無効または未インストール → スキップ ==="
fi
